/* eslint-disable global-require */
export default {
  AW: {
    currency: 'AWG',
    callingCode: '297',
    flag: require('../assets/images/countries/aw.png'),
    name: {
      common: 'Aruba',
      deu: 'Aruba',
      fra: 'Aruba',
      hrv: 'Aruba',
      ita: 'Aruba',
      jpn: 'アルバ',
      nld: 'Aruba',
      por: 'Aruba',
      rus: 'Аруба',
      spa: 'Aruba',
      svk: 'Aruba',
      fin: 'Aruba',
      zho: '阿鲁巴',
      isr: 'ארובה',
      ces: 'Aruba',
      slk: 'Aruba',
      est: 'Aruba',
      pol: 'Aruba',
      urd: 'اروبا',
      kor: '아루바',
    },
  },
  AF: {
    currency: 'AFN',
    callingCode: '93',
    flag: require('../assets/images/countries/af.png'),
    name: {
      common: 'Afghanistan',
      cym: 'Affganistan',
      deu: 'Afghanistan',
      fra: 'Afghanistan',
      hrv: 'Afganistan',
      ita: 'Afghanistan',
      jpn: 'アフガニスタン',
      nld: 'Afghanistan',
      por: 'Afeganistão',
      rus: 'Афганистан',
      spa: 'Afganistán',
      svk: 'Afganistan',
      fin: 'Afganistan',
      zho: '阿富汗',
      isr: 'אפגניסטן',
      ces: 'Afghánistán',
      slk: 'Afganistan',
      est: 'Afganistan',
      pol: 'Afganistan',
      urd: 'افغانستان',
      kor: '아프가니스탄',
    },
  },
  AO: {
    currency: 'AOA',
    callingCode: '244',
    flag: require('../assets/images/countries/ao.png'),
    name: {
      common: 'Angola',
      cym: 'Angola',
      deu: 'Angola',
      fra: 'Angola',
      hrv: 'Angola',
      ita: 'Angola',
      jpn: 'アンゴラ',
      nld: 'Angola',
      por: 'Angola',
      rus: 'Ангола',
      spa: 'Angola',
      svk: 'Angola',
      fin: 'Angola',
      zho: '安哥拉',
      isr: 'אנגולה',
      ces: 'Angola',
      slk: 'Angola',
      est: 'Angola',
      pol: 'Angola',
      urd: 'انگولہ',
      kor: '앙골라',
    },
  },
  AI: {
    currency: 'XCD',
    callingCode: '1264',
    flag: require('../assets/images/countries/ai.png'),
    name: {
      common: 'Anguilla',
      deu: 'Anguilla',
      fra: 'Anguilla',
      hrv: 'Angvila',
      ita: 'Anguilla',
      jpn: 'アンギラ',
      nld: 'Anguilla',
      por: 'Anguilla',
      rus: 'Ангилья',
      spa: 'Anguilla',
      svk: 'Anguilla',
      fin: 'Anguilla',
      zho: '安圭拉',
      isr: 'אנגילה',
      ces: 'Anguilla',
      slk: 'Anguilla',
      est: 'Anguilla',
      pol: 'Anguilla',
      urd: 'اینگویلا',
      kor: '앵귈라',
    },
  },
  AX: {
    currency: 'EUR',
    callingCode: '358',
    flag: require('../assets/images/countries/ax.png'),
    name: {
      common: 'Åland Islands',
      deu: 'Åland',
      fra: 'Ahvenanmaa',
      hrv: 'Ålandski otoci',
      ita: 'Isole Aland',
      jpn: 'オーランド諸島',
      nld: 'Ålandeilanden',
      por: 'Alândia',
      rus: 'Аландские острова',
      spa: 'Alandia',
      svk: 'Alandy',
      fin: 'Ahvenanmaa',
      zho: '奥兰群岛',
      isr: 'איי אולנד',
      ces: 'Ålandy',
      slk: 'Alandy',
      est: 'Ahvenamaa',
      pol: 'Wyspy Alandzkie',
      urd: 'جزائر اولند',
      kor: '올란드 제도',
    },
  },
  AL: {
    currency: 'ALL',
    callingCode: '355',
    flag: require('../assets/images/countries/al.png'),
    name: {
      common: 'Albania',
      cym: 'Albania',
      deu: 'Albanien',
      fra: 'Albanie',
      hrv: 'Albanija',
      ita: 'Albania',
      jpn: 'アルバニア',
      nld: 'Albanië',
      por: 'Albânia',
      rus: 'Албания',
      spa: 'Albania',
      svk: 'Albánsko',
      fin: 'Albania',
      zho: '阿尔巴尼亚',
      isr: 'אלבניה',
      ces: 'Albánie',
      slk: 'Albánsko',
      est: 'Albaania',
      pol: 'Albania',
      urd: 'البانیا',
      kor: '알바니아',
    },
  },
  AD: {
    currency: 'EUR',
    callingCode: '376',
    flag: require('../assets/images/countries/ad.png'),
    name: {
      common: 'Andorra',
      cym: 'Andorra',
      deu: 'Andorra',
      fra: 'Andorre',
      hrv: 'Andora',
      ita: 'Andorra',
      jpn: 'アンドラ',
      nld: 'Andorra',
      por: 'Andorra',
      rus: 'Андорра',
      spa: 'Andorra',
      svk: 'Andorra',
      fin: 'Andorra',
      zho: '安道尔',
      isr: 'אנדורה',
      ces: 'Andorra',
      slk: 'Andorra',
      est: 'Andorra',
      pol: 'Andora',
      urd: 'انڈورا',
      kor: '안도라',
    },
  },
  AE: {
    currency: 'AED',
    callingCode: '971',
    flag: require('../assets/images/countries/ae.png'),
    name: {
      common: 'United Arab Emirates',
      deu: 'Vereinigte Arabische Emirate',
      fra: 'Émirats arabes unis',
      hrv: 'Ujedinjeni Arapski Emirati',
      ita: 'Emirati Arabi Uniti',
      jpn: 'アラブ首長国連邦',
      nld: 'Verenigde Arabische Emiraten',
      por: 'Emirados Árabes Unidos',
      rus: 'Объединённые Арабские Эмираты',
      spa: 'Emiratos Árabes Unidos',
      svk: 'Spojené arabské emiráty',
      fin: 'Arabiemiraatit',
      zho: '阿拉伯联合酋长国',
      isr: 'איחוד האמירויות הערביות',
      ces: 'Spojené arabské emiráty',
      slk: 'Spojené arabské emiráty',
      est: 'Araabia Ühendemiraadid',
      pol: 'Zjednoczone Emiraty Arabskie',
      urd: 'متحدہ عرب امارات',
      kor: '아랍에미리트',
    },
  },
  AR: {
    currency: 'ARS',
    callingCode: '54',
    flag: require('../assets/images/countries/ar.png'),
    name: {
      common: 'Argentina',
      cym: 'Ariannin',
      deu: 'Argentinien',
      fra: 'Argentine',
      hrv: 'Argentina',
      ita: 'Argentina',
      jpn: 'アルゼンチン',
      nld: 'Argentinië',
      por: 'Argentina',
      rus: 'Аргентина',
      spa: 'Argentina',
      svk: 'Argentína',
      fin: 'Argentiina',
      zho: '阿根廷',
      isr: 'ארגנטינה',
      ces: 'Argentina',
      slk: 'Argentína',
      est: 'Argentina',
      pol: 'Argentyna',
      urd: 'ارجنٹائن',
      kor: '아르헨티나',
    },
  },
  AM: {
    currency: 'AMD',
    callingCode: '374',
    flag: require('../assets/images/countries/am.png'),
    name: {
      common: 'Armenia',
      cym: 'Armenia',
      deu: 'Armenien',
      fra: 'Arménie',
      hrv: 'Armenija',
      ita: 'Armenia',
      jpn: 'アルメニア',
      nld: 'Armenië',
      por: 'Arménia',
      rus: 'Армения',
      spa: 'Armenia',
      svk: 'Arménsko',
      fin: 'Armenia',
      zho: '亚美尼亚',
      isr: 'ארמניה',
      ces: 'Arménie',
      slk: 'Arménsko',
      est: 'Armeenia',
      pol: 'Armenia',
      urd: 'آرمینیا',
      kor: '아르메니아',
    },
  },
  AS: {
    currency: 'USD',
    callingCode: '1684',
    flag: require('../assets/images/countries/as.png'),
    name: {
      common: 'American Samoa',
      deu: 'Amerikanisch-Samoa',
      fra: 'Samoa américaines',
      hrv: 'Američka Samoa',
      ita: 'Samoa Americane',
      jpn: 'アメリカ領サモア',
      nld: 'Amerikaans Samoa',
      por: 'Samoa Americana',
      rus: 'Американское Самоа',
      spa: 'Samoa Americana',
      svk: 'Americká Samoa',
      fin: 'Amerikan Samoa',
      zho: '美属萨摩亚',
      isr: 'סמואה האמריקנית',
      ces: 'Americká Samoa',
      slk: 'Americká Samoa',
      est: 'Ameerika Samoa',
      pol: 'Samoa Amerykańskie',
      urd: 'امریکی سمووا',
      kor: '아메리칸사모아',
    },
  },
  AQ: {
    flag: require('../assets/images/countries/aq.png'),
    name: {
      common: 'Antarctica',
      cym: 'Antarctica',
      deu: 'Antarktis',
      fra: 'Antarctique',
      hrv: 'Antarktika',
      ita: 'Antartide',
      jpn: '南極',
      nld: 'Antarctica',
      por: 'Antártida',
      rus: 'Антарктида',
      spa: 'Antártida',
      svk: 'Antarktída',
      fin: 'Etelämanner',
      zho: '南极洲',
      isr: 'אנטארקטיקה',
      ces: 'Antarktida',
      slk: 'Antarktída',
      est: 'Antarktika',
      pol: 'Antarktyka',
      urd: 'انٹارکٹکا',
      kor: '남극',
    },
  },
  TF: {
    currency: 'EUR',
    flag: require('../assets/images/countries/tf.png'),
    name: {
      common: 'French Southern and Antarctic Lands',
      deu: 'Französische Süd-und Antarktisgebiete',
      fra: 'Terres australes et antarctiques françaises',
      hrv: 'Francuski južni i antarktički teritoriji',
      ita: 'Territori Francesi del Sud',
      jpn: 'フランス領南方・南極地域',
      nld: 'Franse Gebieden in de zuidelijke Indische Oceaan',
      por: 'Terras Austrais e Antárticas Francesas',
      rus: 'Французские Южные и Антарктические территории',
      spa: 'Tierras Australes y Antárticas Francesas',
      svk: 'Francúzske juŽné a antarktické územia',
      fin: 'Ranskan eteläiset ja antarktiset alueet',
      zho: '法国南部和南极土地',
      isr: 'הטריטוריות הדרומיות של צרפת',
      ces: 'Francouzská jižní a antarktická území',
      slk: 'Francúzske juŽné a antarktické územia',
      est: 'Prantsuse Lõunaalad',
      pol: 'Francuskie Terytoria Południowe i Antarktyczne',
      urd: 'سرزمین جنوبی فرانسیسیہ و انٹارکٹیکا',
      kor: '프랑스령 남부와 남극 지역',
    },
  },
  AG: {
    currency: 'XCD',
    callingCode: '1268',
    flag: require('../assets/images/countries/ag.png'),
    name: {
      common: 'Antigua and Barbuda',
      cym: 'Antigwa a Barbiwda',
      deu: 'Antigua und Barbuda',
      fra: 'Antigua-et-Barbuda',
      hrv: 'Antigva i Barbuda',
      ita: 'Antigua e Barbuda',
      jpn: 'アンティグア・バーブーダ',
      nld: 'Antigua en Barbuda',
      por: 'Antígua e Barbuda',
      rus: 'Антигуа и Барбуда',
      spa: 'Antigua y Barbuda',
      svk: 'Antigua a Barbuda',
      fin: 'Antigua ja Barbuda',
      zho: '安提瓜和巴布达',
      isr: 'אנטיגואה וברבודה',
      ces: 'Antigua a Barbuda',
      slk: 'Antigua a Barbuda',
      est: 'Antigua ja Barbuda',
      pol: 'Antigua i Barbuda',
      urd: 'اینٹیگوا و باربوڈا',
      kor: '앤티가 바부다',
    },
  },
  AU: {
    currency: 'AUD',
    callingCode: '61',
    flag: require('../assets/images/countries/au.png'),
    name: {
      common: 'Australia',
      cym: 'Awstralia',
      deu: 'Australien',
      fra: 'Australie',
      hrv: 'Australija',
      ita: 'Australia',
      jpn: 'オーストラリア',
      nld: 'Australië',
      por: 'Austrália',
      rus: 'Австралия',
      spa: 'Australia',
      svk: 'Austrália',
      fin: 'Australia',
      zho: '澳大利亚',
      isr: 'אוסטרליה',
      ces: 'Austrálie',
      slk: 'Austrália',
      est: 'Austraalia',
      pol: 'Australia',
      urd: 'آسٹریلیا',
      kor: '호주',
    },
  },
  AT: {
    currency: 'EUR',
    callingCode: '43',
    flag: require('../assets/images/countries/at.png'),
    name: {
      common: 'Austria',
      cym: 'Awstria',
      deu: 'Österreich',
      fra: 'Autriche',
      hrv: 'Austrija',
      ita: 'Austria',
      jpn: 'オーストリア',
      nld: 'Oostenrijk',
      por: 'Áustria',
      rus: 'Австрия',
      spa: 'Austria',
      svk: 'Rakúsko',
      fin: 'Itävalta',
      zho: '奥地利',
      isr: 'אוסטריה',
      ces: 'Rakousko',
      slk: 'Rakúsko',
      est: 'Austria',
      pol: 'Austria',
      urd: 'آسٹریا',
      kor: '오스트리아',
    },
  },
  AZ: {
    currency: 'AZN',
    callingCode: '994',
    flag: require('../assets/images/countries/az.png'),
    name: {
      common: 'Azerbaijan',
      cym: 'Aserbaijan',
      deu: 'Aserbaidschan',
      fra: 'Azerbaïdjan',
      hrv: 'Azerbajdžan',
      ita: 'Azerbaijan',
      jpn: 'アゼルバイジャン',
      nld: 'Azerbeidzjan',
      por: 'Azerbeijão',
      rus: 'Азербайджан',
      spa: 'Azerbaiyán',
      svk: 'Azerbajǆan',
      fin: 'Azerbaidzan',
      zho: '阿塞拜疆',
      isr: 'אזרבייג׳ן',
      ces: 'Ázerbájdžán',
      slk: 'AzerbajǇan',
      est: 'Aserbaidžaan',
      pol: 'Azerbejdżan',
      urd: 'آذربائیجان',
      kor: '아제르바이잔',
    },
  },
  BI: {
    currency: 'BIF',
    callingCode: '257',
    flag: require('../assets/images/countries/bi.png'),
    name: {
      common: 'Burundi',
      cym: 'Bwrwndi',
      deu: 'Burundi',
      fra: 'Burundi',
      hrv: 'Burundi',
      ita: 'Burundi',
      jpn: 'ブルンジ',
      nld: 'Burundi',
      por: 'Burundi',
      rus: 'Бурунди',
      spa: 'Burundi',
      svk: 'Burundi',
      fin: 'Burundi',
      zho: '布隆迪',
      isr: 'בורונדי',
      ces: 'Burundi',
      slk: 'Burundi',
      est: 'Burundi',
      pol: 'Burundi',
      urd: 'برونڈی',
      kor: '부룬디',
    },
  },
  BE: {
    currency: 'EUR',
    callingCode: '32',
    flag: require('../assets/images/countries/be.png'),
    name: {
      common: 'Belgium',
      cym: 'Gwlad Belg',
      deu: 'Belgien',
      fra: 'Belgique',
      hrv: 'Belgija',
      ita: 'Belgio',
      jpn: 'ベルギー',
      nld: 'België',
      por: 'Bélgica',
      rus: 'Бельгия',
      spa: 'Bélgica',
      svk: 'Belgicko',
      fin: 'Belgia',
      zho: '比利时',
      isr: 'בלגיה',
      ces: 'Belgie',
      slk: 'Belgicko',
      est: 'Belgia',
      pol: 'Belgia',
      urd: 'بلجئیم',
      kor: '벨기에',
    },
  },
  BJ: {
    currency: 'XOF',
    callingCode: '229',
    flag: require('../assets/images/countries/bj.png'),
    name: {
      common: 'Benin',
      cym: 'Benin',
      deu: 'Benin',
      fra: 'Bénin',
      hrv: 'Benin',
      ita: 'Benin',
      jpn: 'ベナン',
      nld: 'Benin',
      por: 'Benin',
      rus: 'Бенин',
      spa: 'Benín',
      svk: 'Benin',
      fin: 'Benin',
      zho: '贝宁',
      isr: 'בנין',
      ces: 'Benin',
      slk: 'Benin',
      est: 'Benin',
      pol: 'Benin',
      urd: 'بینن',
      kor: '베냉',
    },
  },
  BF: {
    currency: 'XOF',
    callingCode: '226',
    flag: require('../assets/images/countries/bf.png'),
    name: {
      common: 'Burkina Faso',
      cym: 'Burkina Faso',
      deu: 'Burkina Faso',
      fra: 'Burkina Faso',
      hrv: 'Burkina Faso',
      ita: 'Burkina Faso',
      jpn: 'ブルキナファソ',
      nld: 'Burkina Faso',
      por: 'Burkina Faso',
      rus: 'Буркина-Фасо',
      spa: 'Burkina Faso',
      svk: 'Burkina Faso',
      fin: 'Burkina Faso',
      zho: '布基纳法索',
      isr: 'בורקינה פאסו',
      ces: 'Burkina Faso',
      slk: 'Burkina Faso',
      est: 'Burkina Faso',
      pol: 'Burkina Faso',
      urd: 'برکینا فاسو',
      kor: '부르키나파소',
    },
  },
  BD: {
    currency: 'BDT',
    callingCode: '880',
    flag: require('../assets/images/countries/bd.png'),
    name: {
      common: 'Bangladesh',
      cym: 'Bangladesh',
      deu: 'Bangladesch',
      fra: 'Bangladesh',
      hrv: 'Bangladeš',
      ita: 'Bangladesh',
      jpn: 'バングラデシュ',
      nld: 'Bangladesh',
      por: 'Bangladesh',
      rus: 'Бангладеш',
      spa: 'Bangladesh',
      svk: 'Bangladéš',
      fin: 'Bangladesh',
      zho: '孟加拉国',
      isr: 'בנגלדש',
      ces: 'Bangladéš',
      slk: 'Bangladéš',
      est: 'Bangladesh',
      pol: 'Bangladesz',
      urd: 'بنگلہ دیش',
      kor: '방글라데시',
    },
  },
  BG: {
    currency: 'BGN',
    callingCode: '359',
    flag: require('../assets/images/countries/bg.png'),
    name: {
      common: 'Bulgaria',
      cym: 'Bwlgaria',
      deu: 'Bulgarien',
      fra: 'Bulgarie',
      hrv: 'Bugarska',
      ita: 'Bulgaria',
      jpn: 'ブルガリア',
      nld: 'Bulgarije',
      por: 'Bulgária',
      rus: 'Болгария',
      spa: 'Bulgaria',
      svk: 'Bulharsko',
      fin: 'Bulgaria',
      zho: '保加利亚',
      isr: 'בולגריה',
      ces: 'Bulharsko',
      slk: 'Bulharsko',
      est: 'Bulgaaria',
      pol: 'Bułgaria',
      urd: 'بلغاریہ',
      kor: '불가리아',
    },
  },
  BH: {
    currency: 'BHD',
    callingCode: '973',
    flag: require('../assets/images/countries/bh.png'),
    name: {
      common: 'Bahrain',
      cym: 'Bahrain',
      deu: 'Bahrain',
      fra: 'Bahreïn',
      hrv: 'Bahrein',
      ita: 'Bahrein',
      jpn: 'バーレーン',
      nld: 'Bahrein',
      por: 'Bahrein',
      rus: 'Бахрейн',
      spa: 'Bahrein',
      svk: 'Bahrajn',
      fin: 'Bahrain',
      zho: '巴林',
      isr: 'בחריין',
      ces: 'Bahrajn',
      slk: 'Bahrajn',
      est: 'Bahrein',
      pol: 'Bahrajn',
      urd: 'بحرین',
      kor: '바레인',
    },
  },
  BS: {
    currency: 'BSD',
    callingCode: '1242',
    flag: require('../assets/images/countries/bs.png'),
    name: {
      common: 'Bahamas',
      cym: 'Bahamas',
      deu: 'Bahamas',
      fra: 'Bahamas',
      hrv: 'Bahami',
      ita: 'Bahamas',
      jpn: 'バハマ',
      nld: 'Bahama’s',
      por: 'Bahamas',
      rus: 'Багамские Острова',
      spa: 'Bahamas',
      svk: 'Bahamy',
      fin: 'Bahamasaaret',
      zho: '巴哈马',
      isr: 'איי בהאמה',
      ces: 'Bahamy',
      slk: 'Bahamy',
      est: 'Bahama',
      pol: 'Bahamy',
      urd: 'بہاماس',
      kor: '바하마',
    },
  },
  BA: {
    currency: 'BAM',
    callingCode: '387',
    flag: require('../assets/images/countries/ba.png'),
    name: {
      common: 'Bosnia and Herzegovina',
      cym: 'Bosnia a Hercegovina',
      deu: 'Bosnien und Herzegowina',
      fra: 'Bosnie-Herzégovine',
      hrv: 'Bosna i Hercegovina',
      ita: 'Bosnia ed Erzegovina',
      jpn: 'ボスニア・ヘルツェゴビナ',
      nld: 'Bosnië en Herzegovina',
      por: 'Bósnia e Herzegovina',
      rus: 'Босния и Герцеговина',
      spa: 'Bosnia y Herzegovina',
      svk: 'Bosna a Hercegovina',
      fin: 'Bosnia ja Hertsegovina',
      zho: '波斯尼亚和黑塞哥维那',
      isr: 'בוסניה והרצגובינה',
      ces: 'Bosna a Hercegovina',
      slk: 'Bosna a Hercegovina',
      est: 'Bosnia ja Hertsegoviina',
      pol: 'Bośnia i Hercegowina',
      urd: 'بوسنیا و ہرزیگووینا',
      kor: '보스니아 헤르체고비나',
    },
  },
  BL: {
    currency: 'EUR',
    callingCode: '590',
    flag: require('../assets/images/countries/bl.png'),
    name: {
      common: 'Saint Barthélemy',
      deu: 'Saint-Barthélemy',
      fra: 'Saint-Barthélemy',
      hrv: 'Saint Barthélemy',
      ita: 'Antille Francesi',
      jpn: 'サン・バルテルミー',
      nld: 'Saint Barthélemy',
      por: 'São Bartolomeu',
      rus: 'Сен-Бартелеми',
      spa: 'San Bartolomé',
      svk: 'Svätý Bartolomej',
      fin: 'Saint-Barthélemy',
      zho: '圣巴泰勒米',
      isr: 'סנט ברתולומיאו',
      ces: 'Svatý Bartoloměj',
      slk: 'Svätý Bartolomej',
      est: 'Saint-Barthélemy',
      pol: 'Saint-Barthélemy',
      urd: 'سینٹ بارتھیملے',
      kor: '생바르텔레미',
    },
  },
  BY: {
    currency: 'BYR',
    callingCode: '375',
    flag: require('../assets/images/countries/by.png'),
    name: {
      common: 'Belarus',
      cym: 'Belarws',
      deu: 'Weißrussland',
      fra: 'Biélorussie',
      hrv: 'Bjelorusija',
      ita: 'Bielorussia',
      jpn: 'ベラルーシ',
      nld: 'Wit-Rusland',
      por: 'Bielorússia',
      rus: 'Белоруссия',
      spa: 'Bielorrusia',
      svk: 'Bielorusko',
      fin: 'Valko-Venäjä',
      zho: '白俄罗斯',
      isr: 'בלארוס',
      ces: 'Bělorusko',
      slk: 'Bielorusko',
      est: 'Valgevene',
      pol: 'Białoruś',
      urd: 'بیلاروس',
      kor: '벨라루스',
    },
  },
  BZ: {
    currency: 'BZD',
    callingCode: '501',
    flag: require('../assets/images/countries/bz.png'),
    name: {
      common: 'Belize',
      cym: 'Belize',
      deu: 'Belize',
      fra: 'Belize',
      hrv: 'Belize',
      ita: 'Belize',
      jpn: 'ベリーズ',
      nld: 'Belize',
      por: 'Belize',
      rus: 'Белиз',
      spa: 'Belice',
      svk: 'Belize',
      fin: 'Belize',
      zho: '伯利兹',
      isr: 'בליז',
      ces: 'Belize',
      slk: 'Belize',
      est: 'Belize',
      pol: 'Belize',
      urd: 'بیلیز',
      kor: '벨리즈',
    },
  },
  BM: {
    currency: 'BMD',
    callingCode: '1441',
    flag: require('../assets/images/countries/bm.png'),
    name: {
      common: 'Bermuda',
      cym: 'Bermiwda',
      deu: 'Bermuda',
      fra: 'Bermudes',
      hrv: 'Bermudi',
      ita: 'Bermuda',
      jpn: 'バミューダ',
      nld: 'Bermuda',
      por: 'Bermudas',
      rus: 'Бермудские Острова',
      spa: 'Bermudas',
      svk: 'Bermudy',
      fin: 'Bermuda',
      zho: '百慕大',
      isr: 'ברמודה',
      ces: 'Bermudy',
      slk: 'Bermudy',
      est: 'Bermuda',
      pol: 'Bermudy',
      urd: 'برمودا',
      kor: '버뮤다',
    },
  },
  BO: {
    currency: 'BOB',
    callingCode: '591',
    flag: require('../assets/images/countries/bo.png'),
    name: {
      common: 'Bolivia',
      cym: 'Bolifia',
      deu: 'Bolivien',
      fra: 'Bolivie',
      hrv: 'Bolivija',
      ita: 'Bolivia',
      jpn: 'ボリビア多民族国',
      nld: 'Bolivia',
      por: 'Bolívia',
      rus: 'Боливия',
      spa: 'Bolivia',
      svk: 'Bolívija',
      fin: 'Bolivia',
      zho: '玻利维亚',
      isr: 'בוליביה',
      ces: 'Bolívie',
      slk: 'Bolívia',
      est: 'Boliivia',
      pol: 'Boliwia',
      urd: 'بولیویا',
      kor: '볼리비아',
    },
  },
  BR: {
    currency: 'BRL',
    callingCode: '55',
    flag: require('../assets/images/countries/br.png'),
    name: {
      common: 'Brazil',
      cym: 'Brasil',
      deu: 'Brasilien',
      fra: 'Brésil',
      hrv: 'Brazil',
      ita: 'Brasile',
      jpn: 'ブラジル',
      nld: 'Brazilië',
      por: 'Brasil',
      rus: 'Бразилия',
      spa: 'Brasil',
      svk: 'Brazília',
      fin: 'Brasilia',
      zho: '巴西',
      isr: 'ברזיל',
      ces: 'Brazílie',
      slk: 'Brazília',
      est: 'Brasiilia',
      pol: 'Brazylia',
      urd: 'برازیل',
      kor: '브라질',
    },
  },
  BB: {
    currency: 'BBD',
    callingCode: '1246',
    flag: require('../assets/images/countries/bb.png'),
    name: {
      common: 'Barbados',
      cym: 'Barbados',
      deu: 'Barbados',
      fra: 'Barbade',
      hrv: 'Barbados',
      ita: 'Barbados',
      jpn: 'バルバドス',
      nld: 'Barbados',
      por: 'Barbados',
      rus: 'Барбадос',
      spa: 'Barbados',
      svk: 'Barbados',
      fin: 'Barbados',
      zho: '巴巴多斯',
      isr: 'ברבדוס',
      ces: 'Barbados',
      slk: 'Barbados',
      est: 'Barbados',
      pol: 'Barbados',
      urd: 'بارباڈوس',
      kor: '바베이도스',
    },
  },
  BN: {
    currency: 'BND',
    callingCode: '673',
    flag: require('../assets/images/countries/bn.png'),
    name: {
      common: 'Brunei',
      cym: 'Brunei',
      deu: 'Brunei',
      fra: 'Brunei',
      hrv: 'Brunej',
      ita: 'Brunei',
      jpn: 'ブルネイ・ダルサラーム',
      nld: 'Brunei',
      por: 'Brunei',
      rus: 'Бруней',
      spa: 'Brunei',
      svk: 'Brunej',
      fin: 'Brunei',
      zho: '文莱',
      isr: 'ברוניי',
      ces: 'Brunej',
      slk: 'Brunej',
      est: 'Brunei',
      pol: 'Brunei',
      urd: 'برونائی',
      kor: '브루나이',
    },
  },
  BT: {
    currency: 'BTN',
    callingCode: '975',
    flag: require('../assets/images/countries/bt.png'),
    name: {
      common: 'Bhutan',
      cym: 'Bhwtan',
      deu: 'Bhutan',
      fra: 'Bhoutan',
      hrv: 'Butan',
      ita: 'Bhutan',
      jpn: 'ブータン',
      nld: 'Bhutan',
      por: 'Butão',
      rus: 'Бутан',
      spa: 'Bután',
      svk: 'Bhután',
      fin: 'Bhutan',
      zho: '不丹',
      isr: 'בהוטן',
      ces: 'Bhútán',
      slk: 'Bhután',
      est: 'Bhutan',
      pol: 'Bhutan',
      urd: 'بھوٹان',
      kor: '부탄',
    },
  },
  BV: {
    currency: 'NOK',
    flag: require('../assets/images/countries/bv.png'),
    name: {
      common: 'Bouvet Island',
      deu: 'Bouvetinsel',
      fra: 'Île Bouvet',
      hrv: 'Otok Bouvet',
      ita: 'Isola Bouvet',
      jpn: 'ブーベ島',
      nld: 'Bouveteiland',
      por: 'Ilha Bouvet',
      rus: 'Остров Буве',
      spa: 'Isla Bouvet',
      svk: 'Bouvetov ostrov',
      fin: "Bouvet'nsaari",
      zho: '布维岛',
      isr: 'איי בובה',
      ces: 'Bouvetův ostrov',
      slk: 'Bouvetov ostrov',
      est: 'Bouvet’ saar',
      pol: 'Wyspa Bouveta',
      urd: 'جزیرہ بووہ',
      kor: '부베 섬',
    },
  },
  BW: {
    currency: 'BWP',
    callingCode: '267',
    flag: require('../assets/images/countries/bw.png'),
    name: {
      common: 'Botswana',
      deu: 'Botswana',
      fra: 'Botswana',
      hrv: 'Bocvana',
      ita: 'Botswana',
      jpn: 'ボツワナ',
      nld: 'Botswana',
      por: 'Botswana',
      rus: 'Ботсвана',
      spa: 'Botswana',
      svk: 'Botswana',
      fin: 'Botswana',
      zho: '博茨瓦纳',
      isr: 'בוצוואנה',
      ces: 'Botswana',
      slk: 'Botswana',
      est: 'Botswana',
      pol: 'Botswana',
      urd: 'بوٹسوانا',
      kor: '보츠와나',
    },
  },
  CF: {
    currency: 'XAF',
    callingCode: '236',
    flag: require('../assets/images/countries/cf.png'),
    name: {
      common: 'Central African Republic',
      cym: 'Gweriniaeth Canolbarth Affrica',
      deu: 'Zentralafrikanische Republik',
      fra: 'République centrafricaine',
      hrv: 'Srednjoafrička Republika',
      ita: 'Repubblica Centrafricana',
      jpn: '中央アフリカ共和国',
      nld: 'Centraal-Afrikaanse Republiek',
      por: 'República Centro-Africana',
      rus: 'Центральноафриканская Республика',
      spa: 'República Centroafricana',
      svk: 'Stredoafrická republika',
      fin: 'Keski-Afrikan tasavalta',
      zho: '中非共和国',
      isr: 'הרפובליקה של מרכז אפריקה',
      ces: 'Středoafrická republika',
      slk: 'Stredoafrická republika',
      est: 'Kesk-Aafrika Vabariik',
      pol: 'Republika Środkowoafrykańska',
      urd: 'وسطی افریقی جمہوریہ',
      kor: '중앙아프리카 공화국',
    },
  },
  CA: {
    currency: 'CAD',
    callingCode: '1',
    flag: require('../assets/images/countries/ca.png'),
    name: {
      common: 'Canada',
      cym: 'Canada',
      deu: 'Kanada',
      fra: 'Canada',
      hrv: 'Kanada',
      ita: 'Canada',
      jpn: 'カナダ',
      nld: 'Canada',
      por: 'Canadá',
      rus: 'Канада',
      spa: 'Canadá',
      svk: 'Kanada',
      fin: 'Kanada',
      zho: '加拿大',
      isr: 'קנדה',
      ces: 'Kanada',
      slk: 'Kanada',
      est: 'Kanada',
      pol: 'Kanada',
      urd: 'کینیڈا',
      kor: '캐나다',
    },
  },
  CC: {
    currency: 'AUD',
    callingCode: '61',
    flag: require('../assets/images/countries/cc.png'),
    name: {
      common: 'Cocos (Keeling) Islands',
      cym: 'Ynysoedd Cocos',
      deu: 'Kokosinseln',
      fra: 'Îles Cocos',
      hrv: 'Kokosovi Otoci',
      ita: 'Isole Cocos e Keeling',
      jpn: 'ココス（キーリング）諸島',
      nld: 'Cocoseilanden',
      por: 'Ilhas Cocos (Keeling)',
      rus: 'Кокосовые острова',
      spa: 'Islas Cocos o Islas Keeling',
      svk: 'Kokosové ostrovy',
      fin: 'Kookossaaret',
      zho: '科科斯',
      isr: 'איי קוקוס (קילינג)',
      ces: 'Kokosové ostrovy',
      slk: 'Kokosové ostrovy',
      est: 'Kookossaared',
      pol: 'Wyspy Kokosowe',
      urd: 'جزائر کوکوس',
      kor: '코코스 제도',
    },
  },
  CH: {
    currency: 'CHE',
    callingCode: '41',
    flag: require('../assets/images/countries/ch.png'),
    name: {
      common: 'Switzerland',
      deu: 'Schweiz',
      fra: 'Suisse',
      hrv: 'Švicarska',
      ita: 'Svizzera',
      jpn: 'スイス',
      nld: 'Zwitserland',
      por: 'Suíça',
      rus: 'Швейцария',
      spa: 'Suiza',
      svk: 'Švajčiarsko',
      fin: 'Sveitsi',
      zho: '瑞士',
      isr: 'שווייץ',
      ces: 'Švýcarsko',
      slk: 'Švajčiarsko',
      est: 'Šveits',
      pol: 'Szwajcaria',
      urd: 'سویٹذرلینڈ',
      kor: '스위스',
    },
  },
  CL: {
    currency: 'CLF',
    callingCode: '56',
    flag: require('../assets/images/countries/cl.png'),
    name: {
      common: 'Chile',
      cym: 'Chile',
      deu: 'Chile',
      fra: 'Chili',
      hrv: 'Čile',
      ita: 'Cile',
      jpn: 'チリ',
      nld: 'Chili',
      por: 'Chile',
      rus: 'Чили',
      spa: 'Chile',
      svk: 'Čile',
      fin: 'Chile',
      zho: '智利',
      isr: 'צ׳ילה',
      ces: 'Chile',
      slk: 'Čile',
      est: 'Tšiili',
      pol: 'Chile',
      urd: 'چلی',
      kor: '칠레',
    },
  },
  CN: {
    currency: 'CNY',
    callingCode: '86',
    flag: require('../assets/images/countries/cn.png'),
    name: {
      common: 'China',
      cym: 'Tsieina',
      deu: 'China',
      fra: 'Chine',
      hrv: 'Kina',
      ita: 'Cina',
      jpn: '中国',
      nld: 'China',
      por: 'China',
      rus: 'Китай',
      spa: 'China',
      svk: 'Čína',
      fin: 'Kiina',
      isr: 'סין',
      ces: 'Čína',
      slk: 'Čína',
      est: 'Hiina',
      pol: 'Chiny',
      urd: 'چین',
      kor: '중국',
    },
  },
  CI: {
    currency: 'XOF',
    callingCode: '225',
    flag: require('../assets/images/countries/ci.png'),
    name: {
      common: 'Ivory Coast',
      deu: 'Elfenbeinküste',
      fra: "Côte d'Ivoire",
      hrv: 'Obala Bjelokosti',
      ita: "Costa d'Avorio",
      jpn: 'コートジボワール',
      nld: 'Ivoorkust',
      por: 'Costa do Marfim',
      rus: 'Кот-д’Ивуар',
      spa: 'Costa de Marfil',
      svk: 'Pobržie Slonoviny',
      fin: 'Norsunluurannikko',
      zho: '科特迪瓦',
      isr: 'חוף השנהב',
      ces: 'Pobřeží slonoviny',
      slk: 'Pobržie Slonoviny',
      est: 'Elevandiluurannik',
      pol: 'WybrzeŻe Kości Słoniowej',
      urd: 'آئیوری کوسٹ',
      kor: '코트디부아르',
    },
  },
  CM: {
    currency: 'XAF',
    callingCode: '237',
    flag: require('../assets/images/countries/cm.png'),
    name: {
      common: 'Cameroon',
      cym: 'Camerŵn',
      deu: 'Kamerun',
      fra: 'Cameroun',
      hrv: 'Kamerun',
      ita: 'Camerun',
      jpn: 'カメルーン',
      nld: 'Kameroen',
      por: 'Camarões',
      rus: 'Камерун',
      spa: 'Camerún',
      svk: 'Kamerun',
      fin: 'Kamerun',
      zho: '喀麦隆',
      isr: 'קמרון',
      ces: 'Kamerun',
      slk: 'Kamerun',
      est: 'Kamerun',
      pol: 'WybrzeŻe Kości Słoniowej',
      urd: 'کیمرون',
      kor: '카메룬',
    },
  },
  CD: {
    currency: 'CDF',
    callingCode: '243',
    flag: require('../assets/images/countries/cd.png'),
    name: {
      common: 'DR Congo',
      cym: 'Gweriniaeth Ddemocrataidd Congo',
      deu: 'Kongo (Dem. Rep.)',
      fra: 'Congo (Rép. dém.)',
      hrv: 'Kongo, Demokratska Republika',
      ita: 'Congo (Rep. Dem.)',
      jpn: 'コンゴ民主共和国',
      nld: 'Congo (DRC)',
      por: 'República Democrática do Congo',
      rus: 'Демократическая Республика Конго',
      spa: 'Congo (Rep. Dem.)',
      svk: 'Kongo',
      fin: 'Kongon demokraattinen tasavalta',
      zho: '民主刚果',
      isr: 'קונגו - קינשאסה',
      ces: 'DR Kongo',
      slk: 'Kongo',
      est: 'Kongo DV',
      pol: 'Demokratyczna Republika Konga',
      urd: '\nکانگو',
      kor: '콩고 민주 공화국',
    },
  },
  CG: {
    currency: 'XAF',
    callingCode: '242',
    flag: require('../assets/images/countries/cg.png'),
    name: {
      common: 'Republic of the Congo',
      cym: 'Gweriniaeth y Congo',
      deu: 'Kongo',
      fra: 'Congo',
      hrv: 'Kongo',
      ita: 'Congo',
      jpn: 'コンゴ共和国',
      nld: 'Congo',
      por: 'Congo',
      rus: 'Республика Конго',
      spa: 'Congo',
      svk: 'Kongo',
      fin: 'Kongo-Brazzaville',
      zho: '刚果',
      isr: 'קונגו - ברזאויל',
      ces: 'Kongo',
      slk: 'Kongo',
      est: 'Kongo Vabariik',
      pol: 'Kongo',
      urd: 'جمہوریہ کانگو',
      kor: '콩고',
    },
  },
  CK: {
    currency: 'NZD',
    callingCode: '682',
    flag: require('../assets/images/countries/ck.png'),
    name: {
      common: 'Cook Islands',
      cym: 'Ynysoedd Cook',
      deu: 'Cookinseln',
      fra: 'Îles Cook',
      hrv: 'Cookovo Otočje',
      ita: 'Isole Cook',
      jpn: 'クック諸島',
      nld: 'Cookeilanden',
      por: 'Ilhas Cook',
      rus: 'Острова Кука',
      spa: 'Islas Cook',
      svk: 'Cookove ostrovy',
      fin: 'Cookinsaaret',
      zho: '库克群岛',
      isr: 'איי קוק',
      ces: 'Cookovy ostrovy',
      slk: 'Cookove ostrovy',
      est: 'Cooki saared',
      pol: 'Wyspy Cooka',
      urd: 'جزائر کک',
      kor: '쿡 제도',
    },
  },
  CO: {
    currency: 'COP',
    callingCode: '57',
    flag: require('../assets/images/countries/co.png'),
    name: {
      common: 'Colombia',
      cym: 'Colombia',
      deu: 'Kolumbien',
      fra: 'Colombie',
      hrv: 'Kolumbija',
      ita: 'Colombia',
      jpn: 'コロンビア',
      nld: 'Colombia',
      por: 'Colômbia',
      rus: 'Колумбия',
      spa: 'Colombia',
      svk: 'Kolumbia',
      fin: 'Kolumbia',
      zho: '哥伦比亚',
      isr: 'קולומביה',
      ces: 'Kolumbie',
      slk: 'Kolumbia',
      est: 'Colombia',
      pol: 'Kolumbia',
      urd: 'کولمبیا',
      kor: '콜롬비아',
    },
  },
  KM: {
    currency: 'KMF',
    callingCode: '269',
    flag: require('../assets/images/countries/km.png'),
    name: {
      common: 'Comoros',
      cym: 'Comoros',
      deu: 'Union der Komoren',
      fra: 'Comores',
      hrv: 'Komori',
      ita: 'Comore',
      jpn: 'コモロ',
      nld: 'Comoren',
      por: 'Comores',
      rus: 'Коморы',
      spa: 'Comoras',
      svk: 'Komory',
      fin: 'Komorit',
      zho: '科摩罗',
      isr: 'קומורו',
      ces: 'Komory',
      slk: 'Komory',
      est: 'Komoorid',
      pol: 'Komory',
      urd: 'القمری',
      kor: '코모로',
    },
  },
  CV: {
    currency: 'CVE',
    callingCode: '238',
    flag: require('../assets/images/countries/cv.png'),
    name: {
      common: 'Cape Verde',
      cym: 'Cape Verde',
      deu: 'Kap Verde',
      fra: 'Îles du Cap-Vert',
      hrv: 'Zelenortska Republika',
      ita: 'Capo Verde',
      jpn: 'カーボベルデ',
      nld: 'Kaapverdië',
      por: 'Cabo Verde',
      rus: 'Кабо-Верде',
      spa: 'Cabo Verde',
      svk: 'Kapverdy',
      fin: 'Kap Verde',
      zho: '佛得角',
      isr: 'כף ורדה',
      ces: 'Kapverdy',
      slk: 'Kapverdy',
      est: 'Roheneemesaared',
      pol: 'Republika Zielonego Przylądka',
      urd: 'کیپ ورڈی',
      kor: '카보베르데',
    },
  },
  CR: {
    currency: 'CRC',
    callingCode: '506',
    flag: require('../assets/images/countries/cr.png'),
    name: {
      common: 'Costa Rica',
      cym: 'Costa Rica',
      deu: 'Costa Rica',
      fra: 'Costa Rica',
      hrv: 'Kostarika',
      ita: 'Costa Rica',
      jpn: 'コスタリカ',
      nld: 'Costa Rica',
      por: 'Costa Rica',
      rus: 'Коста-Рика',
      spa: 'Costa Rica',
      svk: 'Kostarika',
      fin: 'Costa Rica',
      zho: '哥斯达黎加',
      isr: 'קוסטה ריקה',
      ces: 'Kostarika',
      slk: 'Kostarika',
      est: 'Costa Rica',
      pol: 'Kostaryka',
      urd: 'کوسٹاریکا',
      kor: '코스타리카',
    },
  },
  CU: {
    currency: 'CUC',
    callingCode: '53',
    flag: require('../assets/images/countries/cu.png'),
    name: {
      common: 'Cuba',
      cym: 'Ciwba',
      deu: 'Kuba',
      fra: 'Cuba',
      hrv: 'Kuba',
      ita: 'Cuba',
      jpn: 'キューバ',
      nld: 'Cuba',
      por: 'Cuba',
      rus: 'Куба',
      spa: 'Cuba',
      svk: 'Kuba',
      fin: 'Kuuba',
      zho: '古巴',
      isr: 'קובה',
      ces: 'Kuba',
      slk: 'Kuba',
      est: 'Kuuba',
      pol: 'Kuba',
      urd: 'کیوبا',
      kor: '쿠바',
    },
  },
  CW: {
    currency: 'ANG',
    callingCode: '5999',
    flag: require('../assets/images/countries/cw.png'),
    name: {
      common: 'Curaçao',
      deu: 'Curaçao',
      fra: 'Curaçao',
      nld: 'Curaçao',
      por: 'ilha da Curação',
      rus: 'Кюрасао',
      spa: 'Curazao',
      svk: 'CuraÇao',
      fin: 'Curaçao',
      zho: '库拉索',
      isr: 'קוראסאו',
      ces: 'Curaçao',
      slk: 'Curacao',
      est: 'Curaçao',
      pol: 'Curaçao',
      urd: 'کیوراساؤ',
      kor: '퀴라소',
    },
  },
  CX: {
    currency: 'AUD',
    callingCode: '61',
    flag: require('../assets/images/countries/cx.png'),
    name: {
      common: 'Christmas Island',
      cym: 'Ynys y Nadolig',
      deu: 'Weihnachtsinsel',
      fra: 'Île Christmas',
      hrv: 'Božićni otok',
      ita: 'Isola di Natale',
      jpn: 'クリスマス島',
      nld: 'Christmaseiland',
      por: 'Ilha do Natal',
      rus: 'Остров Рождества',
      spa: 'Isla de Navidad',
      svk: 'Vianočnú ostrov',
      fin: 'Joulusaari',
      zho: '圣诞岛',
      isr: 'האי כריסטמס',
      ces: 'Vánoční ostrov',
      slk: 'Vianočnú ostrov',
      est: 'Jõulusaar',
      pol: 'Wyspa Bożego Narodzenia',
      urd: 'جزیرہ کرسمس',
      kor: '크리스마스 섬',
    },
  },
  KY: {
    currency: 'KYD',
    callingCode: '1345',
    flag: require('../assets/images/countries/ky.png'),
    name: {
      common: 'Cayman Islands',
      cym: 'Ynysoedd_Cayman',
      deu: 'Kaimaninseln',
      fra: 'Îles Caïmans',
      hrv: 'Kajmanski otoci',
      ita: 'Isole Cayman',
      jpn: 'ケイマン諸島',
      nld: 'Caymaneilanden',
      por: 'Ilhas Caimão',
      rus: 'Каймановы острова',
      spa: 'Islas Caimán',
      svk: 'Kajmanie ostrovy',
      fin: 'Caymansaaret',
      zho: '开曼群岛',
      isr: 'איי קיימן',
      ces: 'Kajmanské ostrovy',
      slk: 'Kajmanie ostrovy',
      est: 'Kaimanisaared',
      pol: 'Kajmany',
      urd: 'جزائر کیمین',
      kor: '케이맨 제도',
    },
  },
  CY: {
    currency: 'EUR',
    callingCode: '357',
    flag: require('../assets/images/countries/cy.png'),
    name: {
      common: 'Cyprus',
      cym: 'Cyprus',
      deu: 'Zypern',
      fra: 'Chypre',
      hrv: 'Cipar',
      ita: 'Cipro',
      jpn: 'キプロス',
      nld: 'Cyprus',
      por: 'Chipre',
      rus: 'Кипр',
      spa: 'Chipre',
      svk: 'Cyprus',
      fin: 'Kypros',
      zho: '塞浦路斯',
      isr: 'קפריסין',
      ces: 'Kypr',
      slk: 'Cyprus',
      est: 'Küpros',
      pol: 'Cypr',
      urd: 'قبرص',
      kor: '키프로스',
    },
  },
  CZ: {
    currency: 'CZK',
    callingCode: '420',
    flag: require('../assets/images/countries/cz.png'),
    name: {
      common: 'Czechia',
      cym: 'Y Weriniaeth Tsiec',
      deu: 'Tschechische Republik',
      fra: 'République tchèque',
      hrv: 'Češka',
      ita: 'Repubblica Ceca',
      jpn: 'チェコ',
      nld: 'Tsjechië',
      por: 'República Checa',
      rus: 'Чехия',
      spa: 'República Checa',
      svk: 'Česko',
      fin: 'Tšekki',
      zho: '捷克',
      isr: 'צ׳כיה',
      ces: 'Česko',
      slk: 'Česko',
      est: 'Tšehhi',
      pol: 'Czechy',
      urd: 'چيک',
      kor: '체코',
    },
  },
  DE: {
    currency: 'EUR',
    callingCode: '49',
    flag: require('../assets/images/countries/de.png'),
    name: {
      common: 'Germany',
      deu: 'Deutschland',
      fra: 'Allemagne',
      hrv: 'Njemačka',
      ita: 'Germania',
      jpn: 'ドイツ',
      nld: 'Duitsland',
      por: 'Alemanha',
      rus: 'Германия',
      spa: 'Alemania',
      svk: 'Nemecko',
      fin: 'Saksa',
      zho: '德国',
      isr: 'גרמניה',
      ces: 'Německo',
      slk: 'Nemecko',
      est: 'Saksamaa',
      pol: 'Niemcy',
      urd: 'جرمنی',
      kor: '독일',
    },
  },
  DJ: {
    currency: 'DJF',
    callingCode: '253',
    flag: require('../assets/images/countries/dj.png'),
    name: {
      common: 'Djibouti',
      cym: 'Djibouti',
      deu: 'Dschibuti',
      fra: 'Djibouti',
      hrv: 'Džibuti',
      ita: 'Gibuti',
      jpn: 'ジブチ',
      nld: 'Djibouti',
      por: 'Djibouti',
      rus: 'Джибути',
      spa: 'Djibouti',
      svk: 'ǅibutsko',
      fin: 'Dijibouti',
      zho: '吉布提',
      isr: 'ג׳יבוטי',
      ces: 'Džibutsko',
      slk: 'ǅibutsko',
      est: 'Djibouti',
      pol: 'Dżibuti',
      urd: 'جبوتی',
      kor: '지부티',
    },
  },
  DM: {
    currency: 'XCD',
    callingCode: '1767',
    flag: require('../assets/images/countries/dm.png'),
    name: {
      common: 'Dominica',
      cym: 'Dominica',
      deu: 'Dominica',
      fra: 'Dominique',
      hrv: 'Dominika',
      ita: 'Dominica',
      jpn: 'ドミニカ国',
      nld: 'Dominica',
      por: 'Dominica',
      rus: 'Доминика',
      spa: 'Dominica',
      svk: 'Dominika',
      fin: 'Dominica',
      zho: '多米尼加',
      isr: 'דומיניקה',
      ces: 'Dominika',
      slk: 'Dominika',
      est: 'Dominica',
      pol: 'Dominika',
      urd: 'ڈومینیکا',
      kor: '도미니카 공화국',
    },
  },
  DK: {
    currency: 'DKK',
    callingCode: '45',
    flag: require('../assets/images/countries/dk.png'),
    name: {
      common: 'Denmark',
      cym: 'Denmarc',
      deu: 'Dänemark',
      fra: 'Danemark',
      hrv: 'Danska',
      ita: 'Danimarca',
      jpn: 'デンマーク',
      nld: 'Denemarken',
      por: 'Dinamarca',
      rus: 'Дания',
      spa: 'Dinamarca',
      svk: 'Dánsko',
      fin: 'Tanska',
      zho: '丹麦',
      isr: 'דנמרק',
      ces: 'Dánsko',
      slk: 'Dánsko',
      est: 'Taani',
      pol: 'Dania',
      urd: 'ڈنمارک',
      kor: '덴마크',
    },
  },
  DO: {
    currency: 'DOP',
    callingCode: '1809',
    flag: require('../assets/images/countries/do.png'),
    name: {
      common: 'Dominican Republic',
      cym: 'Gweriniaeth_Dominica',
      deu: 'Dominikanische Republik',
      fra: 'République dominicaine',
      hrv: 'Dominikanska Republika',
      ita: 'Repubblica Dominicana',
      jpn: 'ドミニカ共和国',
      nld: 'Dominicaanse Republiek',
      por: 'República Dominicana',
      rus: 'Доминиканская Республика',
      spa: 'República Dominicana',
      svk: 'Dominikánska republika',
      fin: 'Dominikaaninen tasavalta',
      zho: '多明尼加',
      isr: 'הרפובליקה הדומיניקנית',
      ces: 'Dominikánská republika',
      slk: 'Dominikánska republika',
      est: 'Dominikaani Vabariik',
      pol: 'Dominikana',
      urd: 'ڈومینیکن',
      kor: '도미니카 공화국',
    },
  },
  DZ: {
    currency: 'DZD',
    callingCode: '213',
    flag: require('../assets/images/countries/dz.png'),
    name: {
      common: 'Algeria',
      cym: 'Algeria',
      deu: 'Algerien',
      fra: 'Algérie',
      hrv: 'Alžir',
      ita: 'Algeria',
      jpn: 'アルジェリア',
      nld: 'Algerije',
      por: 'Argélia',
      rus: 'Алжир',
      spa: 'Argelia',
      svk: 'Alžírsko',
      fin: 'Algeria',
      zho: '阿尔及利亚',
      isr: 'אלג׳יריה',
      ces: 'Alžírsko',
      slk: 'Alžírsko',
      est: 'Alžeeria',
      pol: 'Algieria',
      urd: 'الجزائر',
      kor: '알제리',
    },
  },
  EC: {
    currency: 'USD',
    callingCode: '593',
    flag: require('../assets/images/countries/ec.png'),
    name: {
      common: 'Ecuador',
      cym: 'Ecwador',
      deu: 'Ecuador',
      fra: 'Équateur',
      hrv: 'Ekvador',
      ita: 'Ecuador',
      jpn: 'エクアドル',
      nld: 'Ecuador',
      por: 'Equador',
      rus: 'Эквадор',
      spa: 'Ecuador',
      svk: 'Ekvádor',
      fin: 'Ecuador',
      zho: '厄瓜多尔',
      isr: 'אקוודור',
      ces: 'Ekvádor',
      slk: 'Ekvádor',
      est: 'Ecuador',
      pol: 'Ekwador',
      urd: 'ایکواڈور',
      kor: '에콰도르',
    },
  },
  EG: {
    currency: 'EGP',
    callingCode: '20',
    flag: require('../assets/images/countries/eg.png'),
    name: {
      common: 'Egypt',
      cym: 'Yr Aifft',
      deu: 'Ägypten',
      fra: 'Égypte',
      hrv: 'Egipat',
      ita: 'Egitto',
      jpn: 'エジプト',
      nld: 'Egypte',
      por: 'Egito',
      rus: 'Египет',
      spa: 'Egipto',
      svk: 'Egypt',
      fin: 'Egypti',
      zho: '埃及',
      isr: 'מצרים',
      ces: 'Egypt',
      slk: 'Egypt',
      est: 'Egiptus',
      pol: 'Egipt',
      urd: 'مصر',
      kor: '이집트',
    },
  },
  ER: {
    currency: 'ERN',
    callingCode: '291',
    flag: require('../assets/images/countries/er.png'),
    name: {
      common: 'Eritrea',
      cym: 'Eritrea',
      deu: 'Eritrea',
      fra: 'Érythrée',
      hrv: 'Eritreja',
      ita: 'Eritrea',
      jpn: 'エリトリア',
      nld: 'Eritrea',
      por: 'Eritreia',
      rus: 'Эритрея',
      spa: 'Eritrea',
      svk: 'Eritrea',
      fin: 'Eritrea',
      zho: '厄立特里亚',
      isr: 'אריתריאה',
      ces: 'Eritrea',
      slk: 'Eritrea',
      est: 'Eritrea',
      pol: 'Erytrea',
      urd: 'ارتریا',
      kor: '에리트레아',
    },
  },
  EH: {
    currency: 'MAD',
    callingCode: '212',
    flag: require('../assets/images/countries/eh.png'),
    name: {
      common: 'Western Sahara',
      deu: 'Westsahara',
      fra: 'Sahara Occidental',
      hrv: 'Zapadna Sahara',
      ita: 'Sahara Occidentale',
      jpn: '西サハラ',
      nld: 'Westelijke Sahara',
      por: 'Saara Ocidental',
      rus: 'Западная Сахара',
      spa: 'Sahara Occidental',
      svk: 'Západná Sahara',
      fin: 'Länsi-Sahara',
      zho: '西撒哈拉',
      isr: 'סהרה המערבית',
      ces: 'Západní Sahara',
      slk: 'Západná Sahara',
      est: 'Lääne-Sahara',
      pol: 'Sahara Zachodnia',
      urd: 'مغربی صحارا',
      kor: '서사하라',
    },
  },
  ES: {
    currency: 'EUR',
    callingCode: '34',
    flag: require('../assets/images/countries/es.png'),
    name: {
      common: 'Spain',
      deu: 'Spanien',
      fra: 'Espagne',
      hrv: 'Španjolska',
      ita: 'Spagna',
      jpn: 'スペイン',
      nld: 'Spanje',
      por: 'Espanha',
      rus: 'Испания',
      spa: 'España',
      svk: 'Španielsko',
      fin: 'Espanja',
      zho: '西班牙',
      isr: 'ספרד',
      ces: 'Španělsko',
      slk: 'Španielsko',
      est: 'Hispaania',
      pol: 'Hiszpania',
      urd: 'ہسپانیہ',
      kor: '스페인',
    },
  },
  EE: {
    currency: 'EUR',
    callingCode: '372',
    flag: require('../assets/images/countries/ee.png'),
    name: {
      common: 'Estonia',
      cym: 'Estonia',
      deu: 'Estland',
      fra: 'Estonie',
      hrv: 'Estonija',
      ita: 'Estonia',
      jpn: 'エストニア',
      nld: 'Estland',
      por: 'Estónia',
      rus: 'Эстония',
      spa: 'Estonia',
      svk: 'Estónsko',
      fin: 'Viro',
      zho: '爱沙尼亚',
      isr: 'אסטוניה',
      ces: 'Estonsko',
      slk: 'Estónsko',
      est: 'Eesti',
      pol: 'Estonia',
      urd: 'اسٹونیا',
      kor: '에스토니아',
    },
  },
  ET: {
    currency: 'ETB',
    callingCode: '251',
    flag: require('../assets/images/countries/et.png'),
    name: {
      common: 'Ethiopia',
      cym: 'Ethiopia',
      deu: 'Äthiopien',
      fra: 'Éthiopie',
      hrv: 'Etiopija',
      ita: 'Etiopia',
      jpn: 'エチオピア',
      nld: 'Ethiopië',
      por: 'Etiópia',
      rus: 'Эфиопия',
      spa: 'Etiopía',
      svk: 'Etiópia',
      fin: 'Etiopia',
      zho: '埃塞俄比亚',
      isr: 'אתיופיה',
      ces: 'Etiopie',
      slk: 'Etiópia',
      est: 'Etioopia',
      pol: 'Etiopia',
      urd: 'ایتھوپیا',
      kor: '에티오피아',
    },
  },
  FI: {
    currency: 'EUR',
    callingCode: '358',
    flag: require('../assets/images/countries/fi.png'),
    name: {
      common: 'Finland',
      deu: 'Finnland',
      fra: 'Finlande',
      hrv: 'Finska',
      ita: 'Finlandia',
      jpn: 'フィンランド',
      nld: 'Finland',
      por: 'Finlândia',
      rus: 'Финляндия',
      spa: 'Finlandia',
      svk: 'Fínsko',
      fin: 'Suomi',
      zho: '芬兰',
      isr: 'פינלנד',
      ces: 'Finsko',
      slk: 'Fínsko',
      est: 'Soome',
      pol: 'Finlandia',
      urd: 'فن لینڈ',
      kor: '핀란드',
    },
  },
  FJ: {
    currency: 'FJD',
    callingCode: '679',
    flag: require('../assets/images/countries/fj.png'),
    name: {
      common: 'Fiji',
      deu: 'Fidschi',
      fra: 'Fidji',
      hrv: 'Fiđi',
      ita: 'Figi',
      jpn: 'フィジー',
      nld: 'Fiji',
      por: 'Fiji',
      rus: 'Фиджи',
      spa: 'Fiyi',
      svk: 'Fiǆi',
      fin: 'Fidži',
      zho: '斐济',
      isr: 'פיג׳י',
      ces: 'Fidži',
      slk: 'Fiǆi',
      est: 'Fidži',
      pol: 'Fidżi',
      urd: 'فجی',
      kor: '피지',
    },
  },
  FK: {
    currency: 'FKP',
    callingCode: '500',
    flag: require('../assets/images/countries/fk.png'),
    name: {
      common: 'Falkland Islands',
      deu: 'Falklandinseln',
      fra: 'Îles Malouines',
      hrv: 'Falklandski Otoci',
      ita: 'Isole Falkland o Isole Malvine',
      jpn: 'フォークランド（マルビナス）諸島',
      nld: 'Falklandeilanden',
      por: 'Ilhas Malvinas',
      rus: 'Фолклендские острова',
      spa: 'Islas Malvinas',
      svk: 'Falklandy',
      fin: 'Falkandinsaaret',
      zho: '福克兰群岛',
      isr: 'איי פוקלנד',
      ces: 'Falklandy',
      slk: 'Falklandy',
      est: 'Falklandi saared',
      pol: 'Falklandy',
      urd: 'جزائر فاکلینڈ',
      kor: '포클랜드 제도',
    },
  },
  FR: {
    currency: 'EUR',
    callingCode: '33',
    flag: require('../assets/images/countries/fr.png'),
    name: {
      common: 'France',
      deu: 'Frankreich',
      fra: 'France',
      hrv: 'Francuska',
      ita: 'Francia',
      jpn: 'フランス',
      nld: 'Frankrijk',
      por: 'França',
      rus: 'Франция',
      spa: 'Francia',
      svk: 'Francúzsko',
      fin: 'Ranska',
      zho: '法国',
      isr: 'צרפת',
      ces: 'Francie',
      slk: 'Francúzsko',
      est: 'Prantsusmaa',
      pol: 'Francja',
      urd: 'فرانس',
      kor: '프랑스',
    },
  },
  FO: {
    currency: 'DKK',
    callingCode: '298',
    flag: require('../assets/images/countries/fo.png'),
    name: {
      common: 'Faroe Islands',
      deu: 'Färöer-Inseln',
      fra: 'Îles Féroé',
      hrv: 'Farski Otoci',
      ita: 'Isole Far Oer',
      jpn: 'フェロー諸島',
      nld: 'Faeröer',
      por: 'Ilhas Faroé',
      rus: 'Фарерские острова',
      spa: 'Islas Faroe',
      svk: 'Faerské ostrovy',
      fin: 'Färsaaret',
      zho: '法罗群岛',
      isr: 'איי פארו',
      ces: 'Faerské ostrovy',
      slk: 'Faerské ostrovy',
      est: 'Fääri saared',
      pol: 'Wyspy Owcze',
      urd: 'جزائر فارو',
      kor: '페로 제도',
    },
  },
  FM: {
    currency: 'USD',
    callingCode: '691',
    flag: require('../assets/images/countries/fm.png'),
    name: {
      common: 'Micronesia',
      deu: 'Mikronesien',
      fra: 'Micronésie',
      hrv: 'Mikronezija',
      ita: 'Micronesia',
      jpn: 'ミクロネシア連邦',
      nld: 'Micronesië',
      por: 'Micronésia',
      rus: 'Федеративные Штаты Микронезии',
      spa: 'Micronesia',
      svk: 'Mikronézia',
      fin: 'Mikronesia',
      zho: '密克罗尼西亚',
      isr: 'מיקרונזיה',
      ces: 'Mikronésie',
      slk: 'Mikronézia',
      est: 'Mikroneesia',
      pol: 'Mikronezja',
      urd: 'مائکرونیشیا',
      kor: '미크로네시아',
    },
  },
  GA: {
    currency: 'XAF',
    callingCode: '241',
    flag: require('../assets/images/countries/ga.png'),
    name: {
      common: 'Gabon',
      deu: 'Gabun',
      fra: 'Gabon',
      hrv: 'Gabon',
      ita: 'Gabon',
      jpn: 'ガボン',
      nld: 'Gabon',
      por: 'Gabão',
      rus: 'Габон',
      spa: 'Gabón',
      svk: 'Gabon',
      fin: 'Gabon',
      zho: '加蓬',
      isr: 'גבון',
      ces: 'Gabon',
      slk: 'Gabon',
      est: 'Gabon',
      pol: 'Gabon',
      urd: 'گیبون',
      kor: '가봉',
    },
  },
  GB: {
    currency: 'GBP',
    callingCode: '44',
    flag: require('../assets/images/countries/gb.png'),
    name: {
      common: 'United Kingdom',
      deu: 'Vereinigtes Königreich',
      fra: 'Royaume-Uni',
      hrv: 'Ujedinjeno Kraljevstvo',
      ita: 'Regno Unito',
      jpn: 'イギリス',
      nld: 'Verenigd Koninkrijk',
      por: 'Reino Unido',
      rus: 'Великобритания',
      spa: 'Reino Unido',
      svk: 'Veľká Británia (Spojené kráľovstvo)',
      fin: 'Yhdistynyt kuningaskunta',
      zho: '英国',
      isr: 'הממלכה המאוחדת',
      ces: 'Spojené království',
      slk: 'Veľká Británia (Spojené kráľovstvo)',
      est: 'Suurbritannia',
      pol: 'Zjednoczone Krłlestwo',
      urd: 'مملکتِ متحدہ',
      kor: '영국',
    },
  },
  GE: {
    currency: 'GEL',
    callingCode: '995',
    flag: require('../assets/images/countries/ge.png'),
    name: {
      common: 'Georgia',
      deu: 'Georgien',
      fra: 'Géorgie',
      hrv: 'Gruzija',
      ita: 'Georgia',
      jpn: 'グルジア',
      nld: 'Georgië',
      por: 'Geórgia',
      rus: 'Грузия',
      spa: 'Georgia',
      svk: 'Gruzínsko',
      fin: 'Georgia',
      zho: '格鲁吉亚',
      isr: 'גאורגיה',
      ces: 'Gruzie',
      slk: 'Gruzínsko',
      est: 'Gruusia',
      pol: 'Gruzja',
      urd: 'جارجیا',
      kor: '조지아',
    },
  },
  GG: {
    currency: 'GBP',
    callingCode: '44',
    flag: require('../assets/images/countries/gg.png'),
    name: {
      common: 'Guernsey',
      deu: 'Guernsey',
      fra: 'Guernesey',
      hrv: 'Guernsey',
      ita: 'Guernsey',
      jpn: 'ガーンジー',
      nld: 'Guernsey',
      por: 'Guernsey',
      rus: 'Гернси',
      spa: 'Guernsey',
      svk: 'Guernsey',
      fin: 'Guernsey',
      zho: '根西岛',
      isr: 'גרנסי',
      ces: 'Guernsey',
      slk: 'Guernsey',
      est: 'Guernsey',
      pol: 'Guernsey',
      urd: 'گرنزی',
      kor: '건지 섬',
    },
  },
  GH: {
    currency: 'GHS',
    callingCode: '233',
    flag: require('../assets/images/countries/gh.png'),
    name: {
      common: 'Ghana',
      deu: 'Ghana',
      fra: 'Ghana',
      hrv: 'Gana',
      ita: 'Ghana',
      jpn: 'ガーナ',
      nld: 'Ghana',
      por: 'Gana',
      rus: 'Гана',
      spa: 'Ghana',
      svk: 'Ghana',
      fin: 'Ghana',
      zho: '加纳',
      isr: 'גאנה',
      ces: 'Ghana',
      slk: 'Ghana',
      est: 'Ghana',
      pol: 'Ghana',
      urd: 'گھانا',
      kor: '가나',
    },
  },
  GI: {
    currency: 'GIP',
    callingCode: '350',
    flag: require('../assets/images/countries/gi.png'),
    name: {
      common: 'Gibraltar',
      deu: 'Gibraltar',
      fra: 'Gibraltar',
      hrv: 'Gibraltar',
      ita: 'Gibilterra',
      jpn: 'ジブラルタル',
      nld: 'Gibraltar',
      por: 'Gibraltar',
      rus: 'Гибралтар',
      spa: 'Gibraltar',
      svk: 'Gibraltár',
      fin: 'Gibraltar',
      zho: '直布罗陀',
      isr: 'גיברלטר',
      ces: 'Gibraltar',
      slk: 'Gibraltár',
      est: 'Gibraltar',
      pol: 'Gibraltar',
      urd: 'جبل الطارق',
      kor: '지브롤터',
    },
  },
  GN: {
    currency: 'GNF',
    callingCode: '224',
    flag: require('../assets/images/countries/gn.png'),
    name: {
      common: 'Guinea',
      deu: 'Guinea',
      fra: 'Guinée',
      hrv: 'Gvineja',
      ita: 'Guinea',
      jpn: 'ギニア',
      nld: 'Guinee',
      por: 'Guiné',
      rus: 'Гвинея',
      spa: 'Guinea',
      svk: 'Guinea',
      fin: 'Guinea',
      zho: '几内亚',
      isr: 'גינאה',
      ces: 'Guinea',
      slk: 'Guinea',
      est: 'Guinea',
      pol: 'Gwinea',
      urd: 'گنی',
      kor: '기니',
    },
  },
  GP: {
    currency: 'EUR',
    callingCode: '590',
    flag: require('../assets/images/countries/gp.png'),
    name: {
      common: 'Guadeloupe',
      deu: 'Guadeloupe',
      fra: 'Guadeloupe',
      hrv: 'Gvadalupa',
      ita: 'Guadeloupa',
      jpn: 'グアドループ',
      nld: 'Guadeloupe',
      por: 'Guadalupe',
      rus: 'Гваделупа',
      spa: 'Guadalupe',
      svk: 'Guadeloupe',
      fin: 'Guadeloupe',
      zho: '瓜德罗普岛',
      isr: 'גוואדלופ',
      ces: 'Guadeloupe',
      slk: 'Guadeloupe',
      est: 'Guadeloupe',
      pol: 'Gwadelupa',
      urd: 'گواڈیلوپ',
      kor: '과들루프',
    },
  },
  GM: {
    currency: 'GMD',
    callingCode: '220',
    flag: require('../assets/images/countries/gm.png'),
    name: {
      common: 'Gambia',
      deu: 'Gambia',
      fra: 'Gambie',
      hrv: 'Gambija',
      ita: 'Gambia',
      jpn: 'ガンビア',
      nld: 'Gambia',
      por: 'Gâmbia',
      rus: 'Гамбия',
      spa: 'Gambia',
      svk: 'Gambia',
      fin: 'Gambia',
      zho: '冈比亚',
      isr: 'גמביה',
      ces: 'Gambie',
      slk: 'Gambia',
      est: 'Gambia',
      pol: 'Gambia',
      urd: 'گیمبیا',
      kor: '감비아',
    },
  },
  GW: {
    currency: 'XOF',
    callingCode: '245',
    flag: require('../assets/images/countries/gw.png'),
    name: {
      common: 'Guinea-Bissau',
      deu: 'Guinea-Bissau',
      fra: 'Guinée-Bissau',
      hrv: 'Gvineja Bisau',
      ita: 'Guinea-Bissau',
      jpn: 'ギニアビサウ',
      nld: 'Guinee-Bissau',
      por: 'Guiné-Bissau',
      rus: 'Гвинея-Бисау',
      spa: 'Guinea-Bisáu',
      svk: 'Guinea-Bissau',
      fin: 'Guinea-Bissau',
      zho: '几内亚比绍',
      isr: 'גינאה ביסאו',
      ces: 'Guinea-Bissau',
      slk: 'Guinea-Bissau',
      est: 'Guinea-Bissau',
      pol: 'Gwinea Bissau',
      urd: 'گنی بساؤ',
      kor: '기니비사우',
    },
  },
  GQ: {
    currency: 'XAF',
    callingCode: '240',
    flag: require('../assets/images/countries/gq.png'),
    name: {
      common: 'Equatorial Guinea',
      cym: 'Gini Gyhydeddol',
      deu: 'Äquatorialguinea',
      fra: 'Guinée équatoriale',
      hrv: 'Ekvatorijalna Gvineja',
      ita: 'Guinea Equatoriale',
      jpn: '赤道ギニア',
      nld: 'Equatoriaal-Guinea',
      por: 'Guiné Equatorial',
      rus: 'Экваториальная Гвинея',
      spa: 'Guinea Ecuatorial',
      svk: 'Rovníková Guinea',
      fin: 'Päiväntasaajan Guinea',
      zho: '赤道几内亚',
      isr: 'גינאה המשוונית',
      ces: 'Rovníková Guinea',
      slk: 'Rovníková Guinea',
      est: 'Ekvatoriaal-Guinea',
      pol: 'Gwinea Równikowa',
      urd: 'استوائی گنی',
      kor: '적도 기니',
    },
  },
  GR: {
    currency: 'EUR',
    callingCode: '30',
    flag: require('../assets/images/countries/gr.png'),
    name: {
      common: 'Greece',
      deu: 'Griechenland',
      fra: 'Grèce',
      hrv: 'Grčka',
      ita: 'Grecia',
      jpn: 'ギリシャ',
      nld: 'Griekenland',
      por: 'Grécia',
      rus: 'Греция',
      spa: 'Grecia',
      svk: 'Greécko',
      fin: 'Kreikka',
      zho: '希腊',
      isr: 'יוון',
      ces: 'Řecko',
      slk: 'Greécko',
      est: 'Kreeka',
      pol: 'Grecja',
      urd: 'یونان',
      kor: '그리스',
    },
  },
  GD: {
    currency: 'XCD',
    callingCode: '1473',
    flag: require('../assets/images/countries/gd.png'),
    name: {
      common: 'Grenada',
      deu: 'Grenada',
      fra: 'Grenade',
      hrv: 'Grenada',
      ita: 'Grenada',
      jpn: 'グレナダ',
      nld: 'Grenada',
      por: 'Granada',
      rus: 'Гренада',
      spa: 'Grenada',
      svk: 'Grenada',
      fin: 'Grenada',
      zho: '格林纳达',
      isr: 'גרנדה',
      ces: 'Grenada',
      slk: 'Grenada',
      est: 'Grenada',
      pol: 'Grenada',
      urd: 'گریناڈا',
      kor: '그레나다',
    },
  },
  GL: {
    currency: 'DKK',
    callingCode: '299',
    flag: require('../assets/images/countries/gl.png'),
    name: {
      common: 'Greenland',
      deu: 'Grönland',
      fra: 'Groenland',
      hrv: 'Grenland',
      ita: 'Groenlandia',
      jpn: 'グリーンランド',
      nld: 'Groenland',
      por: 'Gronelândia',
      rus: 'Гренландия',
      spa: 'Groenlandia',
      svk: 'Grónsko',
      fin: 'Groönlanti',
      zho: '格陵兰',
      isr: 'גרינלנד',
      ces: 'Grónsko',
      slk: 'Grónsko',
      est: 'Gröönimaa',
      pol: 'Grenlandia',
      urd: 'گرین لینڈ',
      kor: '그린란드',
    },
  },
  GT: {
    currency: 'GTQ',
    callingCode: '502',
    flag: require('../assets/images/countries/gt.png'),
    name: {
      common: 'Guatemala',
      deu: 'Guatemala',
      fra: 'Guatemala',
      hrv: 'Gvatemala',
      ita: 'Guatemala',
      jpn: 'グアテマラ',
      nld: 'Guatemala',
      por: 'Guatemala',
      rus: 'Гватемала',
      spa: 'Guatemala',
      svk: 'Guatemala',
      fin: 'Guatemala',
      zho: '危地马拉',
      isr: 'גואטמלה',
      ces: 'Guatemala',
      slk: 'Guatemala',
      est: 'Guatemala',
      pol: 'Gwatemala',
      urd: 'گواتیمالا',
      kor: '과테말라',
    },
  },
  GF: {
    currency: 'EUR',
    callingCode: '594',
    flag: require('../assets/images/countries/gf.png'),
    name: {
      common: 'French Guiana',
      deu: 'Französisch Guyana',
      fra: 'Guyane',
      hrv: 'Francuska Gvajana',
      ita: 'Guyana francese',
      jpn: 'フランス領ギアナ',
      nld: 'Frans-Guyana',
      por: 'Guiana Francesa',
      rus: 'Французская Гвиана',
      spa: 'Guayana Francesa',
      svk: 'Guyana',
      fin: 'Ranskan Guayana',
      zho: '法属圭亚那',
      isr: 'גיאנה הצרפתית',
      ces: 'Francouzská Guyana',
      slk: 'Guyana',
      est: 'Prantsuse Guajaana',
      pol: 'Gujana Francuska',
      urd: 'فرانسیسی گیانا',
      kor: '프랑스령 기아나',
    },
  },
  GU: {
    currency: 'USD',
    callingCode: '1671',
    flag: require('../assets/images/countries/gu.png'),
    name: {
      common: 'Guam',
      deu: 'Guam',
      fra: 'Guam',
      hrv: 'Guam',
      ita: 'Guam',
      jpn: 'グアム',
      nld: 'Guam',
      por: 'Guam',
      rus: 'Гуам',
      spa: 'Guam',
      svk: 'Guam',
      fin: 'Guam',
      zho: '关岛',
      isr: 'גואם',
      ces: 'Guam',
      slk: 'Guam',
      est: 'Guam',
      pol: 'Guam',
      urd: 'گوام',
      kor: '괌',
    },
  },
  GY: {
    currency: 'GYD',
    callingCode: '592',
    flag: require('../assets/images/countries/gy.png'),
    name: {
      common: 'Guyana',
      deu: 'Guyana',
      fra: 'Guyana',
      hrv: 'Gvajana',
      ita: 'Guyana',
      jpn: 'ガイアナ',
      nld: 'Guyana',
      por: 'Guiana',
      rus: 'Гайана',
      spa: 'Guyana',
      svk: 'Guyana',
      fin: 'Guayana',
      zho: '圭亚那',
      isr: 'גיאנה',
      ces: 'Guyana',
      slk: 'Guyana',
      est: 'Guyana',
      pol: 'Gujana',
      urd: 'گیانا',
      kor: '가이아나',
    },
  },
  HK: {
    currency: 'HKD',
    callingCode: '852',
    flag: require('../assets/images/countries/hk.png'),
    name: {
      common: 'Hong Kong',
      deu: 'Hongkong',
      fra: 'Hong Kong',
      hrv: 'Hong Kong',
      ita: 'Hong Kong',
      jpn: '香港',
      nld: 'Hongkong',
      por: 'Hong Kong',
      rus: 'Гонконг',
      spa: 'Hong Kong',
      svk: 'Hongkong',
      fin: 'Hongkong',
      isr: 'הונג קונג (מחוז מנהלי מיוחד של סין)',
      ces: 'Hongkong',
      slk: 'Hongkong',
      est: 'Hongkong',
      pol: 'Hongkong',
      urd: 'ہانگ کانگ',
      kor: '홍콩',
    },
  },
  HM: {
    currency: 'AUD',
    flag: require('../assets/images/countries/hm.png'),
    name: {
      common: 'Heard Island and McDonald Islands',
      deu: 'Heard und die McDonaldinseln',
      fra: 'Îles Heard-et-MacDonald',
      hrv: 'Otok Heard i otočje McDonald',
      ita: 'Isole Heard e McDonald',
      jpn: 'ハード島とマクドナルド諸島',
      nld: 'Heard-en McDonaldeilanden',
      por: 'Ilha Heard e Ilhas McDonald',
      rus: 'Остров Херд и острова Макдональд',
      spa: 'Islas Heard y McDonald',
      svk: 'Heardov ostrov',
      fin: 'Heard ja McDonaldinsaaret',
      zho: '赫德岛和麦当劳群岛',
      isr: 'איי הרד ומקדונלד',
      ces: 'Heardův ostrov a McDonaldovy ostrovy',
      slk: 'Heardov ostrov',
      est: 'Heard ja McDonald',
      pol: 'Wyspy Heard i McDonalda',
      urd: 'جزیرہ ہرڈ و جزائر مکڈونلڈ',
      kor: '허드 맥도널드 제도',
    },
  },
  HN: {
    currency: 'HNL',
    callingCode: '504',
    flag: require('../assets/images/countries/hn.png'),
    name: {
      common: 'Honduras',
      deu: 'Honduras',
      fra: 'Honduras',
      hrv: 'Honduras',
      ita: 'Honduras',
      jpn: 'ホンジュラス',
      nld: 'Honduras',
      por: 'Honduras',
      rus: 'Гондурас',
      spa: 'Honduras',
      fin: 'Honduras',
      zho: '洪都拉斯',
      isr: 'הונדורס',
      ces: 'Honduras',
      slk: 'Honduras',
      est: 'Honduras',
      pol: 'Honduras',
      urd: 'ہونڈوراس',
      kor: '온두라스',
    },
  },
  HR: {
    currency: 'HRK',
    callingCode: '385',
    flag: require('../assets/images/countries/hr.png'),
    name: {
      common: 'Croatia',
      cym: 'Croatia',
      deu: 'Kroatien',
      fra: 'Croatie',
      hrv: 'Hrvatska',
      ita: 'Croazia',
      jpn: 'クロアチア',
      nld: 'Kroatië',
      por: 'Croácia',
      rus: 'Хорватия',
      spa: 'Croacia',
      svk: 'Chorvátsko',
      fin: 'Kroatia',
      zho: '克罗地亚',
      isr: 'קרואטיה',
      ces: 'Chorvatsko',
      slk: 'Chorvátsko',
      est: 'Horvaatia',
      pol: 'Chorwacja',
      urd: 'کرویئشا',
      kor: '크로아티아',
    },
  },
  HT: {
    currency: 'HTG',
    callingCode: '509',
    flag: require('../assets/images/countries/ht.png'),
    name: {
      common: 'Haiti',
      deu: 'Haiti',
      fra: 'Haïti',
      hrv: 'Haiti',
      ita: 'Haiti',
      jpn: 'ハイチ',
      nld: 'Haïti',
      por: 'Haiti',
      rus: 'Гаити',
      spa: 'Haiti',
      svk: 'Haiti',
      fin: 'Haiti',
      zho: '海地',
      isr: 'האיטי',
      ces: 'Haiti',
      slk: 'Haiti',
      est: 'Haiti',
      pol: 'Haiti',
      urd: 'ہیٹی',
      kor: '아이티',
    },
  },
  HU: {
    currency: 'HUF',
    callingCode: '36',
    flag: require('../assets/images/countries/hu.png'),
    name: {
      common: 'Hungary',
      deu: 'Ungarn',
      fra: 'Hongrie',
      hrv: 'Mađarska',
      ita: 'Ungheria',
      jpn: 'ハンガリー',
      nld: 'Hongarije',
      por: 'Hungria',
      rus: 'Венгрия',
      spa: 'Hungría',
      svk: 'Maďarsko',
      fin: 'Unkari',
      zho: '匈牙利',
      isr: 'הונגריה',
      ces: 'Maďarsko',
      slk: 'Maďarsko',
      est: 'Ungari',
      pol: 'Węgry',
      urd: 'مجارستان',
      kor: '헝가리',
    },
  },
  ID: {
    currency: 'IDR',
    callingCode: '62',
    flag: require('../assets/images/countries/id.png'),
    name: {
      common: 'Indonesia',
      deu: 'Indonesien',
      fra: 'Indonésie',
      hrv: 'Indonezija',
      ita: 'Indonesia',
      jpn: 'インドネシア',
      nld: 'Indonesië',
      por: 'Indonésia',
      rus: 'Индонезия',
      spa: 'Indonesia',
      svk: 'Indonézia',
      fin: 'Indonesia',
      zho: '印度尼西亚',
      isr: 'אינדונזיה',
      ces: 'Indonésie',
      slk: 'Indonézia',
      est: 'Indoneesia',
      pol: 'Indonezja',
      urd: 'انڈونیشیا',
      kor: '인도네시아',
    },
  },
  IM: {
    currency: 'GBP',
    callingCode: '44',
    flag: require('../assets/images/countries/im.png'),
    name: {
      common: 'Isle of Man',
      deu: 'Insel Man',
      fra: 'Île de Man',
      hrv: 'Otok Man',
      ita: 'Isola di Man',
      jpn: 'マン島',
      nld: 'Isle of Man',
      por: 'Ilha de Man',
      rus: 'Остров Мэн',
      spa: 'Isla de Man',
      svk: 'Man',
      fin: 'Mansaari',
      zho: '马恩岛',
      isr: 'האי מאן',
      ces: 'Ostrov Man',
      slk: 'Man',
      est: 'Mani saar',
      pol: 'Wyspa Man',
      urd: 'آئل آف مین',
      kor: '맨섬',
    },
  },
  IN: {
    currency: 'INR',
    callingCode: '91',
    flag: require('../assets/images/countries/in.png'),
    name: {
      common: 'India',
      deu: 'Indien',
      fra: 'Inde',
      hrv: 'Indija',
      ita: 'India',
      jpn: 'インド',
      nld: 'India',
      por: 'Índia',
      rus: 'Индия',
      spa: 'India',
      svk: 'India',
      fin: 'Intia',
      zho: '印度',
      isr: 'הודו',
      ces: 'Indie',
      slk: 'India',
      est: 'India',
      pol: 'Indie',
      urd: 'بھارت',
      kor: '인도',
    },
  },
  IO: {
    currency: 'USD',
    callingCode: '246',
    flag: require('../assets/images/countries/io.png'),
    name: {
      common: 'British Indian Ocean Territory',
      cym: 'Tiriogaeth Brydeinig Cefnfor India',
      deu: 'Britisches Territorium im Indischen Ozean',
      fra: "Territoire britannique de l'océan Indien",
      hrv: 'Britanski Indijskooceanski teritorij',
      ita: "Territorio britannico dell'oceano indiano",
      jpn: 'イギリス領インド洋地域',
      nld: 'Britse Gebieden in de Indische Oceaan',
      por: 'Território Britânico do Oceano Índico',
      rus: 'Британская территория в Индийском океане',
      spa: 'Territorio Británico del Océano Índico',
      svk: 'Britské indickooceánske územie',
      fin: 'Brittiläinen Intian valtameren alue',
      zho: '英属印度洋领地',
      isr: 'הטריטוריה הבריטית באוקיינוס ההודי',
      ces: 'Britské indickooceánské území',
      slk: 'Britské indickooceánske územie',
      est: 'Briti India ookeani ala',
      pol: 'Brytyjskie Terytorium Oceanu Indyjskiego',
      urd: 'برطانوی بحرہند خطہ',
      kor: '인도',
    },
  },
  IE: {
    currency: 'EUR',
    callingCode: '353',
    flag: require('../assets/images/countries/ie.png'),
    name: {
      common: 'Ireland',
      deu: 'Irland',
      fra: 'Irlande',
      hrv: 'Irska',
      ita: 'Irlanda',
      jpn: 'アイルランド',
      nld: 'Ierland',
      por: 'Irlanda',
      rus: 'Ирландия',
      spa: 'Irlanda',
      svk: 'Írsko',
      fin: 'Irlanti',
      zho: '爱尔兰',
      isr: 'אירלנד',
      ces: 'Irsko',
      slk: 'Írsko',
      est: 'Iirimaa',
      pol: 'Irlandia',
      urd: 'جزیرہ آئرلینڈ',
      kor: '아일랜드',
    },
  },
  IR: {
    currency: 'IRR',
    callingCode: '98',
    flag: require('../assets/images/countries/ir.png'),
    name: {
      common: 'Iran',
      deu: 'Iran',
      fra: 'Iran',
      hrv: 'Iran',
      ita: 'Iran',
      jpn: 'イラン・イスラム共和国',
      nld: 'Iran',
      por: 'Irão',
      rus: 'Иран',
      spa: 'Iran',
      svk: 'Irán',
      fin: 'Iran',
      zho: '伊朗',
      isr: 'איראן',
      ces: 'Írán',
      slk: 'Irán',
      est: 'Iraan',
      pol: 'Iran',
      urd: 'ایران',
      kor: '이란',
    },
  },
  IQ: {
    currency: 'IQD',
    callingCode: '964',
    flag: require('../assets/images/countries/iq.png'),
    name: {
      common: 'Iraq',
      deu: 'Irak',
      fra: 'Irak',
      hrv: 'Irak',
      ita: 'Iraq',
      jpn: 'イラク',
      nld: 'Irak',
      por: 'Iraque',
      rus: 'Ирак',
      spa: 'Irak',
      svk: 'Irak',
      fin: 'Irak',
      zho: '伊拉克',
      isr: 'עיראק',
      ces: 'Irák',
      slk: 'Irak',
      est: 'Iraak',
      pol: 'Irak',
      urd: 'عراق',
      kor: '이라크',
    },
  },
  IS: {
    currency: 'ISK',
    callingCode: '354',
    flag: require('../assets/images/countries/is.png'),
    name: {
      common: 'Iceland',
      deu: 'Island',
      fra: 'Islande',
      hrv: 'Island',
      ita: 'Islanda',
      jpn: 'アイスランド',
      nld: 'IJsland',
      por: 'Islândia',
      rus: 'Исландия',
      spa: 'Islandia',
      svk: 'Island',
      fin: 'Islanti',
      zho: '冰岛',
      isr: 'איסלנד',
      ces: 'Island',
      slk: 'Island',
      est: 'Island',
      pol: 'Islandia',
      urd: 'آئس لینڈ',
      kor: '아이슬란드',
    },
  },
  IL: {
    currency: 'ILS',
    callingCode: '972',
    flag: require('../assets/images/countries/il.png'),
    name: {
      common: 'Israel',
      deu: 'Israel',
      fra: 'Israël',
      hrv: 'Izrael',
      ita: 'Israele',
      jpn: 'イスラエル',
      nld: 'Israël',
      por: 'Israel',
      rus: 'Израиль',
      spa: 'Israel',
      svk: 'Izrael',
      fin: 'Israel',
      zho: '以色列',
      isr: 'ישראל',
      ces: 'Izrael',
      slk: 'Izrael',
      est: 'Iisrael',
      pol: 'Izrael',
      urd: 'اسرائیل',
      kor: '이스라엘',
    },
  },
  IT: {
    currency: 'EUR',
    callingCode: '39',
    flag: require('../assets/images/countries/it.png'),
    name: {
      common: 'Italy',
      deu: 'Italien',
      fra: 'Italie',
      hrv: 'Italija',
      ita: 'Italia',
      jpn: 'イタリア',
      nld: 'Italië',
      por: 'Itália',
      rus: 'Италия',
      spa: 'Italia',
      svk: 'Taliansko',
      fin: 'Italia',
      zho: '意大利',
      isr: 'איטליה',
      ces: 'Itálie',
      slk: 'Taliansko',
      est: 'Itaalia',
      pol: 'Włochy',
      urd: 'اطالیہ',
      kor: '이탈리아',
    },
  },
  JM: {
    currency: 'JMD',
    callingCode: '1876',
    flag: require('../assets/images/countries/jm.png'),
    name: {
      common: 'Jamaica',
      deu: 'Jamaika',
      fra: 'Jamaïque',
      hrv: 'Jamajka',
      ita: 'Giamaica',
      jpn: 'ジャマイカ',
      nld: 'Jamaica',
      por: 'Jamaica',
      rus: 'Ямайка',
      spa: 'Jamaica',
      svk: 'Jamajka',
      fin: 'Jamaika',
      zho: '牙买加',
      isr: 'ג׳מייקה',
      ces: 'Jamajka',
      slk: 'Jamajka',
      est: 'Jamaica',
      pol: 'Jamajka',
      urd: 'جمیکا',
      kor: '자메이카',
    },
  },
  JE: {
    currency: 'GBP',
    callingCode: '44',
    flag: require('../assets/images/countries/je.png'),
    name: {
      common: 'Jersey',
      deu: 'Jersey',
      fra: 'Jersey',
      hrv: 'Jersey',
      ita: 'Isola di Jersey',
      jpn: 'ジャージー',
      nld: 'Jersey',
      por: 'Jersey',
      rus: 'Джерси',
      spa: 'Jersey',
      svk: 'Jersey',
      fin: 'Jersey',
      zho: '泽西岛',
      isr: 'ג׳רסי',
      ces: 'Jersey',
      slk: 'Jersey',
      est: 'Jersey',
      pol: 'Jersey',
      urd: 'جرزی',
      kor: '저지 섬',
    },
  },
  JO: {
    currency: 'JOD',
    callingCode: '962',
    flag: require('../assets/images/countries/jo.png'),
    name: {
      common: 'Jordan',
      deu: 'Jordanien',
      fra: 'Jordanie',
      hrv: 'Jordan',
      ita: 'Giordania',
      jpn: 'ヨルダン',
      nld: 'Jordanië',
      por: 'Jordânia',
      rus: 'Иордания',
      spa: 'Jordania',
      svk: 'Jordánsko',
      fin: 'Jordania',
      zho: '约旦',
      isr: 'ירדן',
      ces: 'Jordánsko',
      slk: 'Jordánsko',
      est: 'Jordaania',
      pol: 'Jordania',
      urd: 'اردن',
      kor: '요르단',
    },
  },
  JP: {
    currency: 'JPY',
    callingCode: '81',
    flag: require('../assets/images/countries/jp.png'),
    name: {
      common: 'Japan',
      deu: 'Japan',
      fra: 'Japon',
      hrv: 'Japan',
      ita: 'Giappone',
      jpn: '日本',
      nld: 'Japan',
      por: 'Japão',
      rus: 'Япония',
      spa: 'Japón',
      svk: 'Japonsko',
      fin: 'Japani',
      zho: '日本',
      isr: 'יפן',
      ces: 'Japonsko',
      slk: 'Japonsko',
      est: 'Jaapan',
      pol: 'Japonia',
      urd: 'جاپان',
      kor: '일본',
    },
  },
  KZ: {
    currency: 'KZT',
    callingCode: '76',
    flag: require('../assets/images/countries/kz.png'),
    name: {
      common: 'Kazakhstan',
      deu: 'Kasachstan',
      fra: 'Kazakhstan',
      hrv: 'Kazahstan',
      ita: 'Kazakistan',
      jpn: 'カザフスタン',
      nld: 'Kazachstan',
      por: 'Cazaquistão',
      rus: 'Казахстан',
      spa: 'Kazajistán',
      svk: 'Kazachstan',
      fin: 'Kazakstan',
      zho: '哈萨克斯坦',
      isr: 'קזחסטן',
      ces: 'Kazachstán',
      slk: 'Kazachstan',
      est: 'Kasahstan',
      pol: 'Kazachstan',
      urd: 'قازقستان',
      kor: '카자흐스탄',
    },
  },
  KE: {
    currency: 'KES',
    callingCode: '254',
    flag: require('../assets/images/countries/ke.png'),
    name: {
      common: 'Kenya',
      deu: 'Kenia',
      fra: 'Kenya',
      hrv: 'Kenija',
      ita: 'Kenya',
      jpn: 'ケニア',
      nld: 'Kenia',
      por: 'Quénia',
      rus: 'Кения',
      spa: 'Kenia',
      svk: 'Keňa',
      fin: 'Kenia',
      zho: '肯尼亚',
      isr: 'קניה',
      ces: 'Keňa',
      slk: 'Keňa',
      est: 'Keenia',
      pol: 'Kenia',
      urd: 'کینیا',
      kor: '케냐',
    },
  },
  KG: {
    currency: 'KGS',
    callingCode: '996',
    flag: require('../assets/images/countries/kg.png'),
    name: {
      common: 'Kyrgyzstan',
      deu: 'Kirgisistan',
      fra: 'Kirghizistan',
      hrv: 'Kirgistan',
      ita: 'Kirghizistan',
      jpn: 'キルギス',
      nld: 'Kirgizië',
      por: 'Quirguistão',
      rus: 'Киргизия',
      spa: 'Kirguizistán',
      svk: 'Kirgizsko',
      fin: 'Kirgisia',
      zho: '吉尔吉斯斯坦',
      isr: 'קירגיזסטן',
      ces: 'Kyrgyzstán',
      slk: 'Kirgizsko',
      est: 'Kõrgõzstan',
      pol: 'Kirgistan',
      urd: 'کرغیزستان',
      kor: '키르기스스탄',
    },
  },
  KH: {
    currency: 'KHR',
    callingCode: '855',
    flag: require('../assets/images/countries/kh.png'),
    name: {
      common: 'Cambodia',
      cym: 'Cambodia',
      deu: 'Kambodscha',
      fra: 'Cambodge',
      hrv: 'Kambodža',
      ita: 'Cambogia',
      jpn: 'カンボジア',
      nld: 'Cambodja',
      por: 'Camboja',
      rus: 'Камбоджа',
      spa: 'Camboya',
      svk: 'Kamboǆa',
      fin: 'Kambodža',
      zho: '柬埔寨',
      isr: 'קמבודיה',
      ces: 'Kambodža',
      slk: 'Kamboǆa',
      est: 'Kambodža',
      pol: 'Kambodża',
      urd: 'کمبوڈیا',
      kor: '캄보디아',
    },
  },
  KI: {
    currency: 'AUD',
    callingCode: '686',
    flag: require('../assets/images/countries/ki.png'),
    name: {
      common: 'Kiribati',
      deu: 'Kiribati',
      fra: 'Kiribati',
      hrv: 'Kiribati',
      ita: 'Kiribati',
      jpn: 'キリバス',
      nld: 'Kiribati',
      por: 'Kiribati',
      rus: 'Кирибати',
      spa: 'Kiribati',
      svk: 'Kiribati',
      fin: 'Kiribati',
      zho: '基里巴斯',
      isr: 'קיריבאטי',
      ces: 'Kiribati',
      slk: 'Kiribati',
      est: 'Kiribati',
      pol: 'Kiribati',
      urd: 'کیریباتی',
      kor: '키리바시',
    },
  },
  KN: {
    currency: 'XCD',
    callingCode: '1869',
    flag: require('../assets/images/countries/kn.png'),
    name: {
      common: 'Saint Kitts and Nevis',
      deu: 'Saint Christopher und Nevis',
      fra: 'Saint-Christophe-et-Niévès',
      hrv: 'Sveti Kristof i Nevis',
      ita: 'Saint Kitts e Nevis',
      jpn: 'セントクリストファー・ネイビス',
      nld: 'Saint Kitts en Nevis',
      por: 'São Cristóvão e Nevis',
      rus: 'Сент-Китс и Невис',
      spa: 'San Cristóbal y Nieves',
      svk: 'Svätý Krištof a Nevis',
      fin: 'Saint Kitts ja Nevis',
      zho: '圣基茨和尼维斯',
      isr: 'סנט קיטס ונוויס',
      ces: 'Svatý Kryštof a Nevis',
      slk: 'Svätý Krištof a Nevis',
      est: 'Saint Kitts ja Nevis',
      pol: 'Saint Kitts i Nevis',
      urd: 'سینٹ کیٹز و ناویس',
      kor: '세인트키츠 네비스',
    },
  },
  KR: {
    currency: 'KRW',
    callingCode: '82',
    flag: require('../assets/images/countries/kr.png'),
    name: {
      common: 'South Korea',
      deu: 'Südkorea',
      fra: 'Corée du Sud',
      hrv: 'Južna Koreja',
      ita: 'Corea del Sud',
      jpn: '大韓民国',
      nld: 'Zuid-Korea',
      por: 'Coreia do Sul',
      rus: 'Южная Корея',
      spa: 'Corea del Sur',
      svk: 'Južná Kórea',
      fin: 'Etelä-Korea',
      zho: '韩国',
      isr: 'קוריאה הדרומית',
      ces: 'Jižní Korea',
      slk: 'Južná Kórea',
      est: 'Lõuna-Korea',
      pol: 'Korea Południowa',
      urd: 'جنوبی کوریا',
      kor: '한국',
    },
  },
  XK: {
    currency: 'EUR',
    callingCode: '383',
    flag: require('../assets/images/countries/xk.png'),
    name: {
      common: 'Kosovo',
      deu: 'Kosovo',
      fra: 'Kosovo',
      hrv: 'Kosovo',
      ita: 'Kosovo',
      nld: 'Kosovo',
      por: 'Kosovo',
      rus: 'Республика Косово',
      spa: 'Kosovo',
      svk: 'Kosovo',
      fin: 'Kosovo',
      zho: '科索沃',
      isr: 'קוסובו',
      ces: 'Kosovo',
      slk: 'Kosovo',
      est: 'Kosovo',
      pol: 'Kosowo',
      urd: 'کوسووہ',
      kor: '코소보',
    },
  },
  KW: {
    currency: 'KWD',
    callingCode: '965',
    flag: require('../assets/images/countries/kw.png'),
    name: {
      common: 'Kuwait',
      deu: 'Kuwait',
      fra: 'Koweït',
      hrv: 'Kuvajt',
      ita: 'Kuwait',
      jpn: 'クウェート',
      nld: 'Koeweit',
      por: 'Kuwait',
      rus: 'Кувейт',
      spa: 'Kuwait',
      svk: 'Kuvajt',
      fin: 'Kuwait',
      zho: '科威特',
      isr: 'כווית',
      ces: 'Kuvajt',
      slk: 'Kuvajt',
      est: 'Kuveit',
      pol: 'Kuwejt',
      urd: 'کویت',
      kor: '쿠웨이트',
    },
  },
  LA: {
    currency: 'LAK',
    callingCode: '856',
    flag: require('../assets/images/countries/la.png'),
    name: {
      common: 'Laos',
      deu: 'Laos',
      fra: 'Laos',
      hrv: 'Laos',
      ita: 'Laos',
      jpn: 'ラオス人民民主共和国',
      nld: 'Laos',
      por: 'Laos',
      rus: 'Лаос',
      spa: 'Laos',
      svk: 'Laos',
      fin: 'Laos',
      zho: '老挝',
      isr: 'לאוס',
      ces: 'Laos',
      slk: 'Laos',
      est: 'Laos',
      pol: 'Laos',
      urd: 'لاؤس',
      kor: '라오스',
    },
  },
  LB: {
    currency: 'LBP',
    callingCode: '961',
    flag: require('../assets/images/countries/lb.png'),
    name: {
      common: 'Lebanon',
      deu: 'Libanon',
      fra: 'Liban',
      hrv: 'Libanon',
      ita: 'Libano',
      jpn: 'レバノン',
      nld: 'Libanon',
      por: 'Líbano',
      rus: 'Ливан',
      spa: 'Líbano',
      svk: 'Libanon',
      fin: 'Libanon',
      zho: '黎巴嫩',
      isr: 'לבנון',
      ces: 'Libanon',
      slk: 'Libanon',
      est: 'Liibanon',
      pol: 'Liban',
      urd: 'لبنان',
      kor: '레바논',
    },
  },
  LR: {
    currency: 'LRD',
    callingCode: '231',
    flag: require('../assets/images/countries/lr.png'),
    name: {
      common: 'Liberia',
      deu: 'Liberia',
      fra: 'Liberia',
      hrv: 'Liberija',
      ita: 'Liberia',
      jpn: 'リベリア',
      nld: 'Liberia',
      por: 'Libéria',
      rus: 'Либерия',
      spa: 'Liberia',
      svk: 'Libéria',
      fin: 'Liberia',
      zho: '利比里亚',
      isr: 'ליבריה',
      ces: 'Libérie',
      slk: 'Libéria',
      est: 'Libeeria',
      pol: 'Liberia',
      urd: 'لائبیریا',
      kor: '라이베리아',
    },
  },
  LY: {
    currency: 'LYD',
    callingCode: '218',
    flag: require('../assets/images/countries/ly.png'),
    name: {
      common: 'Libya',
      deu: 'Libyen',
      fra: 'Libye',
      hrv: 'Libija',
      ita: 'Libia',
      jpn: 'リビア',
      nld: 'Libië',
      por: 'Líbia',
      rus: 'Ливия',
      spa: 'Libia',
      svk: 'Líbya',
      fin: 'Libya',
      zho: '利比亚',
      isr: 'לוב',
      ces: 'Libye',
      slk: 'Líbya',
      est: 'Liibüa',
      pol: 'Libia',
      urd: 'لیبیا',
      kor: '리비아',
    },
  },
  LC: {
    currency: 'XCD',
    callingCode: '1758',
    flag: require('../assets/images/countries/lc.png'),
    name: {
      common: 'Saint Lucia',
      deu: 'Saint Lucia',
      fra: 'Sainte-Lucie',
      hrv: 'Sveta Lucija',
      ita: 'Santa Lucia',
      jpn: 'セントルシア',
      nld: 'Saint Lucia',
      por: 'Santa Lúcia',
      rus: 'Сент-Люсия',
      spa: 'Santa Lucía',
      svk: 'Svätá Lucia',
      fin: 'Saint Lucia',
      zho: '圣卢西亚',
      isr: 'סנט לוסיה',
      ces: 'Svatá Lucie',
      slk: 'Svätá Lucia',
      est: 'Saint Lucia',
      pol: 'Saint Lucia',
      urd: 'سینٹ لوسیا',
      kor: '세인트루시아',
    },
  },
  LI: {
    currency: 'CHF',
    callingCode: '423',
    flag: require('../assets/images/countries/li.png'),
    name: {
      common: 'Liechtenstein',
      deu: 'Liechtenstein',
      fra: 'Liechtenstein',
      hrv: 'Lihtenštajn',
      ita: 'Liechtenstein',
      jpn: 'リヒテンシュタイン',
      nld: 'Liechtenstein',
      por: 'Liechtenstein',
      rus: 'Лихтенштейн',
      spa: 'Liechtenstein',
      svk: 'Lichtenštajnsko',
      fin: 'Liechenstein',
      zho: '列支敦士登',
      isr: 'ליכטנשטיין',
      ces: 'Lichtenštejnsko',
      slk: 'Lichtenštajnsko',
      est: 'Liechtenstein',
      pol: 'Liechtenstein',
      urd: 'لیختینستائن',
      kor: '리히텐슈타인',
    },
  },
  LK: {
    currency: 'LKR',
    callingCode: '94',
    flag: require('../assets/images/countries/lk.png'),
    name: {
      common: 'Sri Lanka',
      deu: 'Sri Lanka',
      fra: 'Sri Lanka',
      hrv: 'Šri Lanka',
      ita: 'Sri Lanka',
      jpn: 'スリランカ',
      nld: 'Sri Lanka',
      por: 'Sri Lanka',
      rus: 'Шри-Ланка',
      spa: 'Sri Lanka',
      svk: 'Srí Lanka',
      fin: 'Sri Lanka',
      zho: '斯里兰卡',
      isr: 'סרי לנקה',
      ces: 'Srí Lanka',
      slk: 'Srí Lanka',
      est: 'Sri Lanka',
      pol: 'Sri Lanka',
      urd: 'سری لنکا',
      kor: '스리랑카',
    },
  },
  LS: {
    currency: 'LSL',
    callingCode: '266',
    flag: require('../assets/images/countries/ls.png'),
    name: {
      common: 'Lesotho',
      deu: 'Lesotho',
      fra: 'Lesotho',
      hrv: 'Lesoto',
      ita: 'Lesotho',
      jpn: 'レソト',
      nld: 'Lesotho',
      por: 'Lesoto',
      rus: 'Лесото',
      spa: 'Lesotho',
      svk: 'Lesotho',
      fin: 'Lesotho',
      zho: '莱索托',
      isr: 'לסוטו',
      ces: 'Lesotho',
      slk: 'Lesotho',
      est: 'Lesotho',
      pol: 'Lesotho',
      urd: 'لیسوتھو',
      kor: '레소토',
    },
  },
  LT: {
    currency: 'EUR',
    callingCode: '370',
    flag: require('../assets/images/countries/lt.png'),
    name: {
      common: 'Lithuania',
      deu: 'Litauen',
      fra: 'Lituanie',
      hrv: 'Litva',
      ita: 'Lituania',
      jpn: 'リトアニア',
      nld: 'Litouwen',
      por: 'Lituânia',
      rus: 'Литва',
      spa: 'Lituania',
      svk: 'Litva',
      fin: 'Liettua',
      zho: '立陶宛',
      isr: 'ליטא',
      ces: 'Litva',
      slk: 'Litva',
      est: 'Leedu',
      pol: 'Litwa',
      urd: 'لتھووینیا',
      kor: '리투아니아',
    },
  },
  LU: {
    currency: 'EUR',
    callingCode: '352',
    flag: require('../assets/images/countries/lu.png'),
    name: {
      common: 'Luxembourg',
      deu: 'Luxemburg',
      fra: 'Luxembourg',
      hrv: 'Luksemburg',
      ita: 'Lussemburgo',
      jpn: 'ルクセンブルク',
      nld: 'Luxemburg',
      por: 'Luxemburgo',
      rus: 'Люксембург',
      spa: 'Luxemburgo',
      svk: 'Luxembursko',
      fin: 'Luxemburg',
      zho: '卢森堡',
      isr: 'לוקסמבורג',
      ces: 'Lucembursko',
      slk: 'Luxembursko',
      est: 'Luksemburg',
      pol: 'Luksemburg',
      urd: 'لکسمبرگ',
      kor: '룩셈부르크',
    },
  },
  LV: {
    currency: 'EUR',
    callingCode: '371',
    flag: require('../assets/images/countries/lv.png'),
    name: {
      common: 'Latvia',
      deu: 'Lettland',
      fra: 'Lettonie',
      hrv: 'Latvija',
      ita: 'Lettonia',
      jpn: 'ラトビア',
      nld: 'Letland',
      por: 'Letónia',
      rus: 'Латвия',
      spa: 'Letonia',
      svk: 'Lotyšsko',
      fin: 'Latvia',
      zho: '拉脱维亚',
      isr: 'לטביה',
      ces: 'Lotyšsko',
      slk: 'Lotyšsko',
      est: 'Läti',
      pol: 'Łotwa',
      urd: 'لٹویا',
      kor: '라트비아',
    },
  },
  MO: {
    currency: 'MOP',
    callingCode: '853',
    flag: require('../assets/images/countries/mo.png'),
    name: {
      common: 'Macau',
      deu: 'Macao',
      fra: 'Macao',
      hrv: 'Makao',
      ita: 'Macao',
      jpn: 'マカオ',
      nld: 'Macao',
      por: 'Macau',
      rus: 'Макао',
      spa: 'Macao',
      fin: 'Macao',
      isr: 'מקאו (מחוז מנהלי מיוחד של סין)',
      ces: 'Macao',
      slk: 'Macao',
      est: 'Macau',
      pol: 'Makau',
      urd: 'مکاؤ',
      kor: '마카오',
    },
  },
  MF: {
    currency: 'EUR',
    callingCode: '590',
    flag: require('../assets/images/countries/mf.png'),
    name: {
      common: 'Saint Martin',
      deu: 'Saint Martin',
      fra: 'Saint-Martin',
      hrv: 'Sveti Martin',
      ita: 'Saint Martin',
      jpn: 'サン・マルタン（フランス領）',
      nld: 'Saint-Martin',
      por: 'São Martinho',
      rus: 'Сен-Мартен',
      spa: 'Saint Martin',
      svk: 'Svätý Martin',
      fin: 'Saint-Martin',
      zho: '圣马丁',
      isr: 'סן מרטן',
      ces: 'Svatý Martin (Francie)',
      slk: 'Saint-Martin',
      est: 'Saint-Martin',
      pol: 'Saint-Martin',
      urd: 'سینٹ مارٹن',
      kor: '생마르탱',
    },
  },
  MA: {
    currency: 'MAD',
    callingCode: '212',
    flag: require('../assets/images/countries/ma.png'),
    name: {
      common: 'Morocco',
      deu: 'Marokko',
      fra: 'Maroc',
      hrv: 'Maroko',
      ita: 'Marocco',
      jpn: 'モロッコ',
      nld: 'Marokko',
      por: 'Marrocos',
      rus: 'Марокко',
      spa: 'Marruecos',
      svk: 'Maroko',
      fin: 'Marokko',
      zho: '摩洛哥',
      isr: 'מרוקו',
      ces: 'Maroko',
      slk: 'Maroko',
      est: 'Maroko',
      pol: 'Maroko',
      urd: 'مراکش',
      kor: '모로코',
    },
  },
  MC: {
    currency: 'EUR',
    callingCode: '377',
    flag: require('../assets/images/countries/mc.png'),
    name: {
      common: 'Monaco',
      deu: 'Monaco',
      fra: 'Monaco',
      hrv: 'Monako',
      ita: 'Principato di Monaco',
      jpn: 'モナコ',
      nld: 'Monaco',
      por: 'Mónaco',
      rus: 'Монако',
      spa: 'Mónaco',
      svk: 'Monako',
      fin: 'Monaco',
      zho: '摩纳哥',
      isr: 'מונקו',
      ces: 'Monako',
      slk: 'Monako',
      est: 'Monaco',
      pol: 'Monako',
      urd: 'موناکو',
      kor: '모나코',
    },
  },
  MD: {
    currency: 'MDL',
    callingCode: '373',
    flag: require('../assets/images/countries/md.png'),
    name: {
      common: 'Moldova',
      deu: 'Moldawie',
      fra: 'Moldavie',
      hrv: 'Moldova',
      ita: 'Moldavia',
      jpn: 'モルドバ共和国',
      nld: 'Moldavië',
      por: 'Moldávia',
      rus: 'Молдавия',
      spa: 'Moldavia',
      svk: 'Moldavsko',
      fin: 'Moldova',
      zho: '摩尔多瓦',
      isr: 'מולדובה',
      ces: 'Moldavsko',
      slk: 'Moldavsko',
      est: 'Moldova',
      pol: 'Mołdawia',
      urd: 'مالدووا',
      kor: '몰도바',
    },
  },
  MG: {
    currency: 'MGA',
    callingCode: '261',
    flag: require('../assets/images/countries/mg.png'),
    name: {
      common: 'Madagascar',
      deu: 'Madagaskar',
      fra: 'Madagascar',
      hrv: 'Madagaskar',
      ita: 'Madagascar',
      jpn: 'マダガスカル',
      nld: 'Madagaskar',
      por: 'Madagáscar',
      rus: 'Мадагаскар',
      spa: 'Madagascar',
      svk: 'Madagaskar',
      fin: 'Madagaskar',
      zho: '马达加斯加',
      isr: 'מדגסקר',
      ces: 'Madagaskar',
      slk: 'Madagaskar',
      est: 'Madagaskar',
      pol: 'Madagaskar',
      urd: 'مڈغاسکر',
      kor: '마다가스카르',
    },
  },
  MV: {
    currency: 'MVR',
    callingCode: '960',
    flag: require('../assets/images/countries/mv.png'),
    name: {
      common: 'Maldives',
      deu: 'Malediven',
      fra: 'Maldives',
      hrv: 'Maldivi',
      ita: 'Maldive',
      jpn: 'モルディブ',
      nld: 'Maldiven',
      por: 'Maldivas',
      rus: 'Мальдивы',
      spa: 'Maldivas',
      svk: 'Maldivy',
      fin: 'Malediivit',
      zho: '马尔代夫',
      isr: 'האיים המלדיביים',
      ces: 'Maledivy',
      slk: 'Maldivy',
      est: 'Maldiivid',
      pol: 'Malediwy',
      urd: 'مالدیپ',
      kor: '몰디브',
    },
  },
  MX: {
    currency: 'MXN',
    callingCode: '52',
    flag: require('../assets/images/countries/mx.png'),
    name: {
      common: 'Mexico',
      deu: 'Mexiko',
      fra: 'Mexique',
      hrv: 'Meksiko',
      ita: 'Messico',
      jpn: 'メキシコ',
      nld: 'Mexico',
      por: 'México',
      rus: 'Мексика',
      spa: 'México',
      svk: 'Mexiko',
      fin: 'Meksiko',
      zho: '墨西哥',
      isr: 'מקסיקו',
      ces: 'Mexiko',
      slk: 'Mexiko',
      est: 'Mehhiko',
      pol: 'Meksyk',
      urd: 'میکسیکو',
      kor: '멕시코',
    },
  },
  MH: {
    currency: 'USD',
    callingCode: '692',
    flag: require('../assets/images/countries/mh.png'),
    name: {
      common: 'Marshall Islands',
      deu: 'Marshallinseln',
      fra: 'Îles Marshall',
      hrv: 'Maršalovi Otoci',
      ita: 'Isole Marshall',
      jpn: 'マーシャル諸島',
      nld: 'Marshalleilanden',
      por: 'Ilhas Marshall',
      rus: 'Маршалловы Острова',
      spa: 'Islas Marshall',
      svk: 'Marshallove ostrovy',
      fin: 'Marshallinsaaret',
      zho: '马绍尔群岛',
      isr: 'איי מרשל',
      ces: 'Marshallovy ostrovy',
      slk: 'Marshallove ostrovy',
      est: 'Marshalli Saared',
      pol: 'Wyspy Marshalla',
      urd: 'جزائر مارشل',
      kor: '마셜 제도',
    },
  },
  MK: {
    currency: 'MKD',
    callingCode: '389',
    flag: require('../assets/images/countries/mk.png'),
    name: {
      common: 'Macedonia',
      deu: 'Mazedonien',
      fra: 'Macédoine',
      hrv: 'Makedonija',
      ita: 'Macedonia',
      jpn: 'マケドニア旧ユーゴスラビア共和国',
      nld: 'Macedonië',
      por: 'Macedónia',
      rus: 'Республика Македония',
      spa: 'Macedonia',
      svk: 'Macedónsko',
      fin: 'Makedonia',
      zho: '马其顿',
      isr: 'מקדוניה',
      ces: 'Makedonie',
      slk: 'Macedónsko',
      est: 'Makedoonia',
      pol: 'Macedonia',
      urd: 'مقدونیہ',
      kor: '마케도니아',
    },
  },
  ML: {
    currency: 'XOF',
    callingCode: '223',
    flag: require('../assets/images/countries/ml.png'),
    name: {
      common: 'Mali',
      deu: 'Mali',
      fra: 'Mali',
      hrv: 'Mali',
      ita: 'Mali',
      jpn: 'マリ',
      nld: 'Mali',
      por: 'Mali',
      rus: 'Мали',
      spa: 'Mali',
      svk: 'Mali',
      fin: 'Mali',
      zho: '马里',
      isr: 'מאלי',
      ces: 'Mali',
      slk: 'Mali',
      est: 'Mali',
      pol: 'Mali',
      urd: 'مالی',
      kor: '말리',
    },
  },
  MT: {
    currency: 'EUR',
    callingCode: '356',
    flag: require('../assets/images/countries/mt.png'),
    name: {
      common: 'Malta',
      deu: 'Malta',
      fra: 'Malte',
      hrv: 'Malta',
      ita: 'Malta',
      jpn: 'マルタ',
      nld: 'Malta',
      por: 'Malta',
      rus: 'Мальта',
      spa: 'Malta',
      svk: 'Malta',
      fin: 'Malta',
      zho: '马耳他',
      isr: 'מלטה',
      ces: 'Malta',
      slk: 'Malta',
      est: 'Malta',
      pol: 'Malta',
      urd: 'مالٹا',
      kor: '몰타',
    },
  },
  MM: {
    currency: 'MMK',
    callingCode: '95',
    flag: require('../assets/images/countries/mm.png'),
    name: {
      common: 'Myanmar',
      deu: 'Myanmar',
      fra: 'Birmanie',
      hrv: 'Mijanmar',
      ita: 'Birmania',
      jpn: 'ミャンマー',
      nld: 'Myanmar',
      por: 'Myanmar',
      rus: 'Мьянма',
      spa: 'Myanmar',
      svk: 'Mjanmarsko',
      fin: 'Myanmar',
      zho: '缅甸',
      isr: 'מיאנמר (בורמה)',
      ces: 'Myanmar',
      slk: 'Mjanmarsko',
      est: 'Myanmar',
      pol: 'Mjanma',
      urd: 'میانمار',
      kor: '미얀마',
    },
  },
  ME: {
    currency: 'EUR',
    callingCode: '382',
    flag: require('../assets/images/countries/me.png'),
    name: {
      common: 'Montenegro',
      deu: 'Montenegro',
      fra: 'Monténégro',
      hrv: 'Crna Gora',
      ita: 'Montenegro',
      jpn: 'モンテネグロ',
      nld: 'Montenegro',
      por: 'Montenegro',
      rus: 'Черногория',
      spa: 'Montenegro',
      svk: 'Čierna Hora',
      fin: 'Montenegro',
      zho: '黑山',
      isr: 'מונטנגרו',
      ces: 'Černá Hora',
      slk: 'Čierna Hora',
      est: 'Montenegro',
      pol: 'Czarnogóra',
      urd: 'مونٹینیگرو',
      kor: '몬테네그로',
    },
  },
  MN: {
    currency: 'MNT',
    callingCode: '976',
    flag: require('../assets/images/countries/mn.png'),
    name: {
      common: 'Mongolia',
      deu: 'Mongolei',
      fra: 'Mongolie',
      hrv: 'Mongolija',
      ita: 'Mongolia',
      jpn: 'モンゴル',
      nld: 'Mongolië',
      por: 'Mongólia',
      rus: 'Монголия',
      spa: 'Mongolia',
      svk: 'Mongolsko',
      fin: 'Mongolia',
      zho: '蒙古',
      isr: 'מונגוליה',
      ces: 'Mongolsko',
      slk: 'Mongolsko',
      est: 'Mongoolia',
      pol: 'Mongolia',
      urd: 'منگولیا',
      kor: '몽골국',
    },
  },
  MP: {
    currency: 'USD',
    callingCode: '1670',
    flag: require('../assets/images/countries/mp.png'),
    name: {
      common: 'Northern Mariana Islands',
      deu: 'Nördliche Marianen',
      fra: 'Îles Mariannes du Nord',
      hrv: 'Sjevernomarijanski otoci',
      ita: 'Isole Marianne Settentrionali',
      jpn: '北マリアナ諸島',
      nld: 'Noordelijke Marianeneilanden',
      por: 'Marianas Setentrionais',
      rus: 'Северные Марианские острова',
      spa: 'Islas Marianas del Norte',
      svk: 'Severné Mariány',
      fin: 'Pohjois-Mariaanit',
      zho: '北马里亚纳群岛',
      isr: 'איי מריאנה הצפוניים',
      ces: 'Severní Mariany',
      slk: 'Severné Mariány',
      est: 'Põhja-Mariaanid',
      pol: 'Mariany Północne',
      urd: 'جزائر شمالی ماریانا',
      kor: '북마리아나 제도',
    },
  },
  MZ: {
    currency: 'MZN',
    callingCode: '258',
    flag: require('../assets/images/countries/mz.png'),
    name: {
      common: 'Mozambique',
      deu: 'Mosambik',
      fra: 'Mozambique',
      hrv: 'Mozambik',
      ita: 'Mozambico',
      jpn: 'モザンビーク',
      nld: 'Mozambique',
      por: 'Moçambique',
      rus: 'Мозамбик',
      spa: 'Mozambique',
      svk: 'Mozambik',
      fin: 'Mosambik',
      zho: '莫桑比克',
      isr: 'מוזמביק',
      ces: 'Mosambik',
      slk: 'Mozambik',
      est: 'Mosambiik',
      pol: 'Mozambik',
      urd: 'موزمبیق',
      kor: '모잠비크',
    },
  },
  MR: {
    currency: 'MRO',
    callingCode: '222',
    flag: require('../assets/images/countries/mr.png'),
    name: {
      common: 'Mauritania',
      deu: 'Mauretanien',
      fra: 'Mauritanie',
      hrv: 'Mauritanija',
      ita: 'Mauritania',
      jpn: 'モーリタニア',
      nld: 'Mauritanië',
      por: 'Mauritânia',
      rus: 'Мавритания',
      spa: 'Mauritania',
      svk: 'Mauritánia',
      fin: 'Mauritania',
      zho: '毛里塔尼亚',
      isr: 'מאוריטניה',
      ces: 'Mauritánie',
      slk: 'Mauritánia',
      est: 'Mauritaania',
      pol: 'Mauretania',
      urd: 'موریتانیہ',
      kor: '모리타니',
    },
  },
  MS: {
    currency: 'XCD',
    callingCode: '1664',
    flag: require('../assets/images/countries/ms.png'),
    name: {
      common: 'Montserrat',
      deu: 'Montserrat',
      fra: 'Montserrat',
      hrv: 'Montserrat',
      ita: 'Montserrat',
      jpn: 'モントセラト',
      nld: 'Montserrat',
      por: 'Montserrat',
      rus: 'Монтсеррат',
      spa: 'Montserrat',
      svk: 'Montserrat',
      fin: 'Montserrat',
      zho: '蒙特塞拉特',
      isr: 'מונסראט',
      ces: 'Montserrat',
      slk: 'Montserrat',
      est: 'Montserrat',
      pol: 'Montserrat',
      urd: 'مانٹسریٹ',
      kor: '몬트세랫',
    },
  },
  MQ: {
    currency: 'EUR',
    callingCode: '596',
    flag: require('../assets/images/countries/mq.png'),
    name: {
      common: 'Martinique',
      deu: 'Martinique',
      fra: 'Martinique',
      hrv: 'Martinique',
      ita: 'Martinica',
      jpn: 'マルティニーク',
      nld: 'Martinique',
      por: 'Martinica',
      rus: 'Мартиника',
      spa: 'Martinica',
      svk: 'Martinik',
      fin: 'Martinique',
      zho: '马提尼克',
      isr: 'מרטיניק',
      ces: 'Martinik',
      slk: 'Martinique',
      est: 'Martinique',
      pol: 'Martynika',
      urd: 'مارٹینیک',
      kor: '마르티니크',
    },
  },
  MU: {
    currency: 'MUR',
    callingCode: '230',
    flag: require('../assets/images/countries/mu.png'),
    name: {
      common: 'Mauritius',
      deu: 'Mauritius',
      fra: 'Île Maurice',
      hrv: 'Mauricijus',
      ita: 'Mauritius',
      jpn: 'モーリシャス',
      nld: 'Mauritius',
      por: 'Maurício',
      rus: 'Маврикий',
      spa: 'Mauricio',
      svk: 'Maurícius',
      fin: 'Mauritius',
      zho: '毛里求斯',
      isr: 'מאוריציוס',
      ces: 'Mauricius',
      slk: 'Maurícius',
      est: 'Mauritius',
      pol: 'Mauritius',
      urd: 'موریشس',
      kor: '모리셔스',
    },
  },
  MW: {
    currency: 'MWK',
    callingCode: '265',
    flag: require('../assets/images/countries/mw.png'),
    name: {
      common: 'Malawi',
      deu: 'Malawi',
      fra: 'Malawi',
      hrv: 'Malavi',
      ita: 'Malawi',
      jpn: 'マラウイ',
      nld: 'Malawi',
      por: 'Malawi',
      rus: 'Малави',
      spa: 'Malawi',
      svk: 'Malawi',
      fin: 'Malawi',
      zho: '马拉维',
      isr: 'מלאווי',
      ces: 'Malawi',
      slk: 'Malawi',
      est: 'Malawi',
      pol: 'Malawi',
      urd: 'ملاوی',
      kor: '말라위',
    },
  },
  MY: {
    currency: 'MYR',
    callingCode: '60',
    flag: require('../assets/images/countries/my.png'),
    name: {
      common: 'Malaysia',
      deu: 'Malaysia',
      fra: 'Malaisie',
      hrv: 'Malezija',
      ita: 'Malesia',
      jpn: 'マレーシア',
      nld: 'Maleisië',
      por: 'Malásia',
      rus: 'Малайзия',
      spa: 'Malasia',
      svk: 'Malajzia',
      fin: 'Malesia',
      zho: '马来西亚',
      isr: 'מלזיה',
      ces: 'Malajsie',
      slk: 'Malajzia',
      est: 'Malaisia',
      pol: 'Malezja',
      urd: 'ملائیشیا',
      kor: '말레이시아',
    },
  },
  YT: {
    currency: 'EUR',
    callingCode: '262',
    flag: require('../assets/images/countries/yt.png'),
    name: {
      common: 'Mayotte',
      deu: 'Mayotte',
      fra: 'Mayotte',
      hrv: 'Mayotte',
      ita: 'Mayotte',
      jpn: 'マヨット',
      nld: 'Mayotte',
      por: 'Mayotte',
      rus: 'Майотта',
      spa: 'Mayotte',
      svk: 'Mayotte',
      fin: 'Mayotte',
      zho: '马约特',
      isr: 'מאיוט',
      ces: 'Mayotte',
      slk: 'Mayotte',
      est: 'Mayotte',
      pol: 'Majotta',
      urd: 'مایوٹ',
      kor: '마요트',
    },
  },
  NA: {
    currency: 'NAD',
    callingCode: '264',
    flag: require('../assets/images/countries/na.png'),
    name: {
      common: 'Namibia',
      deu: 'Namibia',
      fra: 'Namibie',
      hrv: 'Namibija',
      ita: 'Namibia',
      jpn: 'ナミビア',
      nld: 'Namibië',
      por: 'Namíbia',
      rus: 'Намибия',
      spa: 'Namibia',
      svk: 'Namíbia',
      fin: 'Namibia',
      zho: '纳米比亚',
      isr: 'נמיביה',
      ces: 'Namibie',
      slk: 'Namíbia',
      est: 'Namiibia',
      pol: 'Namibia',
      urd: 'نمیبیا',
      kor: '나미비아',
    },
  },
  NC: {
    currency: 'XPF',
    callingCode: '687',
    flag: require('../assets/images/countries/nc.png'),
    name: {
      common: 'New Caledonia',
      deu: 'Neukaledonien',
      fra: 'Nouvelle-Calédonie',
      hrv: 'Nova Kaledonija',
      ita: 'Nuova Caledonia',
      jpn: 'ニューカレドニア',
      nld: 'Nieuw-Caledonië',
      por: 'Nova Caledónia',
      rus: 'Новая Каледония',
      spa: 'Nueva Caledonia',
      svk: 'Nová Kaledónia',
      fin: 'Uusi-Kaledonia',
      zho: '新喀里多尼亚',
      isr: 'קלדוניה החדשה',
      ces: 'Nová Kaledonie',
      slk: 'Nová Kaledónia',
      est: 'Uus-Kaledoonia',
      pol: 'Nowa Kaledonia',
      urd: 'نیو کیلیڈونیا',
      kor: '누벨칼레도니',
    },
  },
  NE: {
    currency: 'XOF',
    callingCode: '227',
    flag: require('../assets/images/countries/ne.png'),
    name: {
      common: 'Niger',
      deu: 'Niger',
      fra: 'Niger',
      hrv: 'Niger',
      ita: 'Niger',
      jpn: 'ニジェール',
      nld: 'Niger',
      por: 'Níger',
      rus: 'Нигер',
      spa: 'Níger',
      svk: 'Niger',
      fin: 'Niger',
      zho: '尼日尔',
      isr: 'ניז׳ר',
      ces: 'Niger',
      slk: 'Niger',
      est: 'Niger',
      pol: 'Niger',
      urd: 'نائجر',
      kor: '니제르',
    },
  },
  NF: {
    currency: 'AUD',
    callingCode: '672',
    flag: require('../assets/images/countries/nf.png'),
    name: {
      common: 'Norfolk Island',
      deu: 'Norfolkinsel',
      fra: 'Île Norfolk',
      hrv: 'Otok Norfolk',
      ita: 'Isola Norfolk',
      jpn: 'ノーフォーク島',
      nld: 'Norfolkeiland',
      por: 'Ilha Norfolk',
      rus: 'Норфолк',
      spa: 'Isla de Norfolk',
      svk: 'Norfolk',
      fin: 'Norfolkinsaari',
      zho: '诺福克岛',
      isr: 'איי נורפוק',
      ces: 'Norfolk',
      slk: 'Norfolk',
      est: 'Norfolk',
      pol: 'Wyspa Norfolk',
      urd: 'جزیرہ نورفک',
      kor: '노퍽 섬',
    },
  },
  NG: {
    currency: 'NGN',
    callingCode: '234',
    flag: require('../assets/images/countries/ng.png'),
    name: {
      common: 'Nigeria',
      deu: 'Nigeria',
      fra: 'Nigéria',
      hrv: 'Nigerija',
      ita: 'Nigeria',
      jpn: 'ナイジェリア',
      nld: 'Nigeria',
      por: 'Nigéria',
      rus: 'Нигерия',
      spa: 'Nigeria',
      svk: 'Nigéria',
      fin: 'Nigeria',
      zho: '尼日利亚',
      isr: 'ניגריה',
      ces: 'Nigérie',
      slk: 'Nigéria',
      est: 'Nigeeria',
      pol: 'Nigeria',
      urd: 'نائجیریا',
      kor: '나이지리아',
    },
  },
  NI: {
    currency: 'NIO',
    callingCode: '505',
    flag: require('../assets/images/countries/ni.png'),
    name: {
      common: 'Nicaragua',
      deu: 'Nicaragua',
      fra: 'Nicaragua',
      hrv: 'Nikaragva',
      ita: 'Nicaragua',
      jpn: 'ニカラグア',
      nld: 'Nicaragua',
      por: 'Nicarágua',
      rus: 'Никарагуа',
      spa: 'Nicaragua',
      svk: 'Nikaragua',
      fin: 'Nicaragua',
      zho: '尼加拉瓜',
      isr: 'ניקרגואה',
      ces: 'Nikaragua',
      slk: 'Nikaragua',
      est: 'Nicaragua',
      pol: 'Nikaragua',
      urd: 'نکاراگوا',
      kor: '니카라과',
    },
  },
  NU: {
    currency: 'NZD',
    callingCode: '683',
    flag: require('../assets/images/countries/nu.png'),
    name: {
      common: 'Niue',
      deu: 'Niue',
      fra: 'Niue',
      hrv: 'Niue',
      ita: 'Niue',
      jpn: 'ニウエ',
      nld: 'Niue',
      por: 'Niue',
      rus: 'Ниуэ',
      spa: 'Niue',
      svk: 'Niue',
      fin: 'Niue',
      zho: '纽埃',
      isr: 'ניווה',
      ces: 'Niue',
      slk: 'Niue',
      est: 'Niue',
      pol: 'Niue',
      urd: 'نیووے',
      kor: '니우에',
    },
  },
  NL: {
    currency: 'EUR',
    callingCode: '31',
    flag: require('../assets/images/countries/nl.png'),
    name: {
      common: 'Netherlands',
      deu: 'Niederlande',
      fra: 'Pays-Bas',
      hrv: 'Nizozemska',
      ita: 'Paesi Bassi',
      jpn: 'オランダ',
      nld: 'Nederland',
      por: 'Holanda',
      rus: 'Нидерланды',
      spa: 'Países Bajos',
      svk: 'Holansko',
      fin: 'Alankomaat',
      zho: '荷兰',
      isr: 'הולנד',
      ces: 'Nizozemsko',
      slk: 'Holansko',
      est: 'Holland',
      pol: 'Holandia',
      urd: 'نیدرلینڈز',
      kor: '네덜란드',
    },
  },
  NO: {
    currency: 'NOK',
    callingCode: '47',
    flag: require('../assets/images/countries/no.png'),
    name: {
      common: 'Norway',
      deu: 'Norwegen',
      fra: 'Norvège',
      hrv: 'Norveška',
      ita: 'Norvegia',
      jpn: 'ノルウェー',
      nld: 'Noorwegen',
      por: 'Noruega',
      rus: 'Норвегия',
      spa: 'Noruega',
      svk: 'Nórsko',
      fin: 'Norja',
      zho: '挪威',
      isr: 'נורווגיה',
      ces: 'Norsko',
      slk: 'Nórsko',
      est: 'Norra',
      pol: 'Norwegia',
      urd: 'ناروے',
      kor: '노르웨이',
    },
  },
  NP: {
    currency: 'NPR',
    callingCode: '977',
    flag: require('../assets/images/countries/np.png'),
    name: {
      common: 'Nepal',
      deu: 'Népal',
      fra: 'Népal',
      hrv: 'Nepal',
      ita: 'Nepal',
      jpn: 'ネパール',
      nld: 'Nepal',
      por: 'Nepal',
      rus: 'Непал',
      spa: 'Nepal',
      svk: 'Nepál',
      fin: 'Nepal',
      zho: '尼泊尔',
      isr: 'נפאל',
      ces: 'Nepál',
      slk: 'Nepál',
      est: 'Nepal',
      pol: 'Nepal',
      urd: 'نیپال',
      kor: '네팔',
    },
  },
  NR: {
    currency: 'AUD',
    callingCode: '674',
    flag: require('../assets/images/countries/nr.png'),
    name: {
      common: 'Nauru',
      deu: 'Nauru',
      fra: 'Nauru',
      hrv: 'Nauru',
      ita: 'Nauru',
      jpn: 'ナウル',
      nld: 'Nauru',
      por: 'Nauru',
      rus: 'Науру',
      spa: 'Nauru',
      svk: 'Nauru',
      fin: 'Nauru',
      zho: '瑙鲁',
      isr: 'נאורו',
      ces: 'Nauru',
      slk: 'Nauru',
      est: 'Nauru',
      pol: 'Nauru',
      urd: 'ناورو',
      kor: '나우루',
    },
  },
  NZ: {
    currency: 'NZD',
    callingCode: '64',
    flag: require('../assets/images/countries/nz.png'),
    name: {
      common: 'New Zealand',
      deu: 'Neuseeland',
      fra: 'Nouvelle-Zélande',
      hrv: 'Novi Zeland',
      ita: 'Nuova Zelanda',
      jpn: 'ニュージーランド',
      nld: 'Nieuw-Zeeland',
      por: 'Nova Zelândia',
      rus: 'Новая Зеландия',
      spa: 'Nueva Zelanda',
      svk: 'Nový Zéland',
      fin: 'Uusi-Seelanti',
      zho: '新西兰',
      isr: 'ניו זילנד',
      ces: 'Nový Zéland',
      slk: 'Nový Zéland',
      est: 'Uus-Meremaa',
      pol: 'Nowa Zelandia',
      urd: 'نیوزی لینڈ',
      kor: '뉴질랜드',
    },
  },
  OM: {
    currency: 'OMR',
    callingCode: '968',
    flag: require('../assets/images/countries/om.png'),
    name: {
      common: 'Oman',
      deu: 'Oman',
      fra: 'Oman',
      hrv: 'Oman',
      ita: 'oman',
      jpn: 'オマーン',
      nld: 'Oman',
      por: 'Omã',
      rus: 'Оман',
      spa: 'Omán',
      svk: 'Omán',
      fin: 'Oman',
      zho: '阿曼',
      isr: 'עומאן',
      ces: 'Omán',
      slk: 'Omán',
      est: 'Omaan',
      pol: 'Oman',
      urd: 'عمان',
      kor: '오만',
    },
  },
  PK: {
    currency: 'PKR',
    callingCode: '92',
    flag: require('../assets/images/countries/pk.png'),
    name: {
      common: 'Pakistan',
      deu: 'Pakistan',
      fra: 'Pakistan',
      hrv: 'Pakistan',
      ita: 'Pakistan',
      jpn: 'パキスタン',
      nld: 'Pakistan',
      por: 'Paquistão',
      rus: 'Пакистан',
      spa: 'Pakistán',
      svk: 'Pakistan',
      fin: 'Pakistan',
      zho: '巴基斯坦',
      isr: 'פקיסטן',
      ces: 'Pákistán',
      slk: 'Pakistan',
      est: 'Pakistan',
      pol: 'Pakistan',
      urd: 'پاکستان',
      kor: '파키스탄',
    },
  },
  PA: {
    currency: 'PAB',
    callingCode: '507',
    flag: require('../assets/images/countries/pa.png'),
    name: {
      common: 'Panama',
      deu: 'Panama',
      fra: 'Panama',
      hrv: 'Panama',
      ita: 'Panama',
      jpn: 'パナマ',
      nld: 'Panama',
      por: 'Panamá',
      rus: 'Панама',
      spa: 'Panamá',
      svk: 'Panama',
      fin: 'Panama',
      zho: '巴拿马',
      isr: 'פנמה',
      ces: 'Panama',
      slk: 'Panama',
      est: 'Panama',
      pol: 'Panama',
      urd: 'پاناما',
      kor: '파나마',
    },
  },
  PN: {
    currency: 'NZD',
    callingCode: '64',
    flag: require('../assets/images/countries/pn.png'),
    name: {
      common: 'Pitcairn Islands',
      deu: 'Pitcairn',
      fra: 'Îles Pitcairn',
      hrv: 'Pitcairnovo otočje',
      ita: 'Isole Pitcairn',
      jpn: 'ピトケアン',
      nld: 'Pitcairneilanden',
      por: 'Ilhas Pitcairn',
      rus: 'Острова Питкэрн',
      spa: 'Islas Pitcairn',
      svk: 'Pitcairnove ostrovy',
      fin: 'Pitcairn',
      zho: '皮特凯恩群岛',
      isr: 'איי פיטקרן',
      ces: 'Pitcairnovy ostrovy',
      slk: 'Pitcairnove ostrovy',
      est: 'Pitcairn',
      pol: 'Pitcairn',
      urd: 'جزائر پٹکیرن',
      kor: '핏케언 제도',
    },
  },
  PE: {
    currency: 'PEN',
    callingCode: '51',
    flag: require('../assets/images/countries/pe.png'),
    name: {
      common: 'Peru',
      deu: 'Peru',
      fra: 'Pérou',
      hrv: 'Peru',
      ita: 'Perù',
      jpn: 'ペルー',
      nld: 'Peru',
      por: 'Perú',
      rus: 'Перу',
      spa: 'Perú',
      svk: 'Peru',
      fin: 'Peru',
      zho: '秘鲁',
      isr: 'פרו',
      ces: 'Peru',
      slk: 'Peru',
      est: 'Peruu',
      pol: 'Peru',
      urd: 'پیرو',
      kor: '페루',
    },
  },
  PH: {
    currency: 'PHP',
    callingCode: '63',
    flag: require('../assets/images/countries/ph.png'),
    name: {
      common: 'Philippines',
      deu: 'Philippinen',
      fra: 'Philippines',
      hrv: 'Filipini',
      ita: 'Filippine',
      jpn: 'フィリピン',
      nld: 'Filipijnen',
      por: 'Filipinas',
      rus: 'Филиппины',
      spa: 'Filipinas',
      svk: 'Filipíny',
      fin: 'Filippiinit',
      zho: '菲律宾',
      isr: 'הפיליפינים',
      ces: 'Filipíny',
      slk: 'Filipíny',
      est: 'Filipiinid',
      pol: 'Filipiny',
      urd: 'فلپائن',
      kor: '필리핀',
    },
  },
  PW: {
    currency: 'USD',
    callingCode: '680',
    flag: require('../assets/images/countries/pw.png'),
    name: {
      common: 'Palau',
      deu: 'Palau',
      fra: 'Palaos (Palau)',
      hrv: 'Palau',
      ita: 'Palau',
      jpn: 'パラオ',
      nld: 'Palau',
      por: 'Palau',
      rus: 'Палау',
      spa: 'Palau',
      svk: 'Palau',
      fin: 'Palau',
      zho: '帕劳',
      isr: 'פלאו',
      ces: 'Palau',
      slk: 'Palau',
      est: 'Belau',
      pol: 'Palau',
      urd: 'پلاؤ',
      kor: '팔라우',
    },
  },
  PG: {
    currency: 'PGK',
    callingCode: '675',
    flag: require('../assets/images/countries/pg.png'),
    name: {
      common: 'Papua New Guinea',
      deu: 'Papua-Neuguinea',
      fra: 'Papouasie-Nouvelle-Guinée',
      hrv: 'Papua Nova Gvineja',
      ita: 'Papua Nuova Guinea',
      jpn: 'パプアニューギニア',
      nld: 'Papoea-Nieuw-Guinea',
      por: 'Papua Nova Guiné',
      rus: 'Папуа — Новая Гвинея',
      spa: 'Papúa Nueva Guinea',
      svk: 'Papua-Nová Guinea',
      fin: 'Papua-Uusi-Guinea',
      zho: '巴布亚新几内亚',
      isr: 'פפואה גינאה החדשה',
      ces: 'Papua-Nová Guinea',
      slk: 'Papua-Nová Guinea',
      est: 'Paapua Uus-Guinea',
      pol: 'Papua-Nowa Gwinea',
      urd: 'پاپوا نیو گنی',
      kor: '파푸아뉴기니',
    },
  },
  PL: {
    currency: 'PLN',
    callingCode: '48',
    flag: require('../assets/images/countries/pl.png'),
    name: {
      common: 'Poland',
      deu: 'Polen',
      fra: 'Pologne',
      hrv: 'Poljska',
      ita: 'Polonia',
      jpn: 'ポーランド',
      nld: 'Polen',
      por: 'Polónia',
      rus: 'Польша',
      spa: 'Polonia',
      svk: 'Poľsko',
      fin: 'Puola',
      zho: '波兰',
      isr: 'פולין',
      ces: 'Polsko',
      slk: 'Poľsko',
      est: 'Poola',
      pol: 'Polska',
      urd: 'پولینڈ',
      kor: '폴란드',
    },
  },
  PR: {
    currency: 'USD',
    callingCode: '1787',
    flag: require('../assets/images/countries/pr.png'),
    name: {
      common: 'Puerto Rico',
      deu: 'Puerto Rico',
      fra: 'Porto Rico',
      hrv: 'Portoriko',
      ita: 'Porto Rico',
      jpn: 'プエルトリコ',
      nld: 'Puerto Rico',
      por: 'Porto Rico',
      rus: 'Пуэрто-Рико',
      spa: 'Puerto Rico',
      svk: 'Portoriko',
      fin: 'Puerto Rico',
      zho: '波多黎各',
      isr: 'פוארטו ריקו',
      ces: 'Portoriko',
      slk: 'Portoriko',
      est: 'Puerto Rico',
      pol: 'Portoryko',
      urd: 'پورٹو ریکو',
      kor: '푸에르토리코',
    },
  },
  KP: {
    currency: 'KPW',
    callingCode: '850',
    flag: require('../assets/images/countries/kp.png'),
    name: {
      common: 'North Korea',
      deu: 'Nordkorea',
      fra: 'Corée du Nord',
      hrv: 'Sjeverna Koreja',
      ita: 'Corea del Nord',
      jpn: '朝鮮民主主義人民共和国',
      nld: 'Noord-Korea',
      por: 'Coreia do Norte',
      rus: 'Северная Корея',
      spa: 'Corea del Norte',
      svk: 'Kórejská ľudovodemokratická republika (KĽR, Severná Kó)',
      fin: 'Pohjois-Korea',
      zho: '朝鲜',
      isr: 'קוריאה הצפונית',
      ces: 'Severní Korea',
      slk: 'Kórejská ľudovodemokratická republika (KĽR, Severná Kórea)',
      est: 'Põhja-Korea',
      pol: 'Korea Północna',
      urd: 'شمالی کوریا',
      kor: '조선',
    },
  },
  PT: {
    currency: 'EUR',
    callingCode: '351',
    flag: require('../assets/images/countries/pt.png'),
    name: {
      common: 'Portugal',
      deu: 'Portugal',
      fra: 'Portugal',
      hrv: 'Portugal',
      ita: 'Portogallo',
      jpn: 'ポルトガル',
      nld: 'Portugal',
      por: 'Portugal',
      rus: 'Португалия',
      spa: 'Portugal',
      svk: 'Portugalsko',
      fin: 'Portugali',
      zho: '葡萄牙',
      isr: 'פורטוגל',
      ces: 'Portugalsko',
      slk: 'Portugalsko',
      est: 'Portugal',
      pol: 'Portugalia',
      urd: 'پرتگال',
      kor: '포르투갈',
    },
  },
  PY: {
    currency: 'PYG',
    callingCode: '595',
    flag: require('../assets/images/countries/py.png'),
    name: {
      common: 'Paraguay',
      deu: 'Paraguay',
      fra: 'Paraguay',
      hrv: 'Paragvaj',
      ita: 'Paraguay',
      jpn: 'パラグアイ',
      nld: 'Paraguay',
      por: 'Paraguai',
      rus: 'Парагвай',
      spa: 'Paraguay',
      svk: 'Paraguaj',
      fin: 'Paraguay',
      zho: '巴拉圭',
      isr: 'פרגוואי',
      ces: 'Paraguay',
      slk: 'Paraguaj',
      est: 'Paraguay',
      pol: 'Paragwaj',
      urd: 'پیراگوئے',
      kor: '파라과이',
    },
  },
  PS: {
    currency: 'ILS',
    callingCode: '970',
    flag: require('../assets/images/countries/ps.png'),
    name: {
      common: 'Palestine',
      deu: 'Palästina',
      fra: 'Palestine',
      hrv: 'Palestina',
      ita: 'Palestina',
      jpn: 'パレスチナ',
      nld: 'Palestijnse gebieden',
      por: 'Palestina',
      rus: 'Палестина',
      spa: 'Palestina',
      svk: 'Palestína',
      fin: 'Palestiina',
      zho: '巴勒斯坦',
      isr: 'השטחים הפלסטיניים',
      ces: 'Palestina',
      slk: 'Palestína',
      est: 'Palestiina',
      pol: 'Palestyna',
      urd: 'فلسطین',
      kor: '팔레스타인',
    },
  },
  PF: {
    currency: 'XPF',
    callingCode: '689',
    flag: require('../assets/images/countries/pf.png'),
    name: {
      common: 'French Polynesia',
      deu: 'Französisch-Polynesien',
      fra: 'Polynésie française',
      hrv: 'Francuska Polinezija',
      ita: 'Polinesia Francese',
      jpn: 'フランス領ポリネシア',
      nld: 'Frans-Polynesië',
      por: 'Polinésia Francesa',
      rus: 'Французская Полинезия',
      spa: 'Polinesia Francesa',
      svk: 'Francúzska Polynézia',
      fin: 'Ranskan Polynesia',
      zho: '法属波利尼西亚',
      isr: 'פולינזיה הצרפתית',
      ces: 'Francouzská Polynésie',
      slk: 'Francúzska Polynézia',
      est: 'Prantsuse Polüneesia',
      pol: 'Polinezja Francuska',
      urd: 'فرانسیسی پولینیشیا',
      kor: '프랑스령 폴리네시아',
    },
  },
  QA: {
    currency: 'QAR',
    callingCode: '974',
    flag: require('../assets/images/countries/qa.png'),
    name: {
      common: 'Qatar',
      deu: 'Katar',
      fra: 'Qatar',
      hrv: 'Katar',
      ita: 'Qatar',
      jpn: 'カタール',
      nld: 'Qatar',
      por: 'Catar',
      rus: 'Катар',
      spa: 'Catar',
      svk: 'Katar',
      fin: 'Qatar',
      zho: '卡塔尔',
      isr: 'קטאר',
      ces: 'Katar',
      slk: 'Katar',
      est: 'Katar',
      pol: 'Katar',
      urd: 'قطر',
      kor: '카타르',
    },
  },
  RE: {
    currency: 'EUR',
    callingCode: '262',
    flag: require('../assets/images/countries/re.png'),
    name: {
      common: 'Réunion',
      deu: 'Réunion',
      fra: 'Réunion',
      hrv: 'Réunion',
      ita: 'Riunione',
      jpn: 'レユニオン',
      nld: 'Réunion',
      por: 'Reunião',
      rus: 'Реюньон',
      spa: 'Reunión',
      svk: 'Réunion',
      fin: 'Réunion',
      zho: '留尼旺岛',
      isr: 'ראוניון',
      ces: 'Réunion',
      slk: 'Réunion',
      est: 'Réunion',
      pol: 'Reunion',
      urd: 'رے یونیوں',
      kor: '레위니옹',
    },
  },
  RO: {
    currency: 'RON',
    callingCode: '40',
    flag: require('../assets/images/countries/ro.png'),
    name: {
      common: 'Romania',
      deu: 'Rumänien',
      fra: 'Roumanie',
      hrv: 'Rumunjska',
      ita: 'Romania',
      jpn: 'ルーマニア',
      nld: 'Roemenië',
      por: 'Roménia',
      rus: 'Румыния',
      spa: 'Rumania',
      svk: 'Rumunsko',
      fin: 'Romania',
      zho: '罗马尼亚',
      isr: 'רומניה',
      ces: 'Rumunsko',
      slk: 'Rumunsko',
      est: 'Rumeenia',
      pol: 'Rumunia',
      urd: 'رومانیہ',
      kor: '루마니아',
    },
  },
  RU: {
    currency: 'RUB',
    callingCode: '7',
    flag: require('../assets/images/countries/ru.png'),
    name: {
      common: 'Russia',
      deu: 'Russland',
      fra: 'Russie',
      hrv: 'Rusija',
      ita: 'Russia',
      jpn: 'ロシア連邦',
      nld: 'Rusland',
      por: 'Rússia',
      rus: 'Россия',
      spa: 'Rusia',
      svk: 'Rusko',
      fin: 'Venäjä',
      zho: '俄罗斯',
      isr: 'רוסיה',
      ces: 'Rusko',
      slk: 'Rusko',
      est: 'Venemaa',
      pol: 'Rosja',
      urd: 'روس',
      kor: '러시아',
    },
  },
  RW: {
    currency: 'RWF',
    callingCode: '250',
    flag: require('../assets/images/countries/rw.png'),
    name: {
      common: 'Rwanda',
      deu: 'Ruanda',
      fra: 'Rwanda',
      hrv: 'Ruanda',
      ita: 'Ruanda',
      jpn: 'ルワンダ',
      nld: 'Rwanda',
      por: 'Ruanda',
      rus: 'Руанда',
      spa: 'Ruanda',
      svk: 'Rwanda',
      fin: 'Ruanda',
      zho: '卢旺达',
      isr: 'רואנדה',
      ces: 'Rwanda',
      slk: 'Rwanda',
      est: 'Rwanda',
      pol: 'Rwanda',
      urd: 'روانڈا',
      kor: '르완다',
    },
  },
  SA: {
    currency: 'SAR',
    callingCode: '966',
    flag: require('../assets/images/countries/sa.png'),
    name: {
      common: 'Saudi Arabia',
      deu: 'Saudi-Arabien',
      fra: 'Arabie Saoudite',
      hrv: 'Saudijska Arabija',
      ita: 'Arabia Saudita',
      jpn: 'サウジアラビア',
      nld: 'Saoedi-Arabië',
      por: 'Arábia Saudita',
      rus: 'Саудовская Аравия',
      spa: 'Arabia Saudí',
      svk: 'Saudská Arábia',
      fin: 'Saudi-Arabia',
      zho: '沙特阿拉伯',
      isr: 'ערב הסעודית',
      ces: 'Saúdská Arábie',
      slk: 'Saudská Arábia',
      est: 'Saudi Araabia',
      pol: 'Arabia Saudyjska',
      urd: 'سعودی عرب',
      kor: '사우디아라비아',
    },
  },
  SD: {
    currency: 'SDG',
    callingCode: '249',
    flag: require('../assets/images/countries/sd.png'),
    name: {
      common: 'Sudan',
      deu: 'Sudan',
      fra: 'Soudan',
      hrv: 'Sudan',
      ita: 'Sudan',
      jpn: 'スーダン',
      nld: 'Soedan',
      por: 'Sudão',
      rus: 'Судан',
      spa: 'Sudán',
      svk: 'Sudán',
      fin: 'Sudan',
      zho: '苏丹',
      isr: 'סודן',
      ces: 'Súdán',
      slk: 'Sudán',
      est: 'Sudaan',
      pol: 'Sudan',
      urd: 'سودان',
      kor: '수단',
    },
  },
  SN: {
    currency: 'XOF',
    callingCode: '221',
    flag: require('../assets/images/countries/sn.png'),
    name: {
      common: 'Senegal',
      deu: 'Senegal',
      fra: 'Sénégal',
      hrv: 'Senegal',
      ita: 'Senegal',
      jpn: 'セネガル',
      nld: 'Senegal',
      por: 'Senegal',
      rus: 'Сенегал',
      spa: 'Senegal',
      svk: 'Senegal',
      fin: 'Senegal',
      zho: '塞内加尔',
      isr: 'סנגל',
      ces: 'Senegal',
      slk: 'Senegal',
      est: 'Senegal',
      pol: 'Senegal',
      urd: 'سینیگال',
      kor: '세네갈',
    },
  },
  SG: {
    currency: 'SGD',
    callingCode: '65',
    flag: require('../assets/images/countries/sg.png'),
    name: {
      common: 'Singapore',
      deu: 'Singapur',
      fra: 'Singapour',
      hrv: 'Singapur',
      ita: 'Singapore',
      jpn: 'シンガポール',
      nld: 'Singapore',
      por: 'Singapura',
      rus: 'Сингапур',
      spa: 'Singapur',
      svk: 'Singapur',
      fin: 'Singapore',
      isr: 'סינגפור',
      ces: 'Singapur',
      slk: 'Singapur',
      est: 'Singapur',
      pol: 'Singapur',
      urd: 'سنگاپور',
      kor: '싱가포르',
    },
  },
  GS: {
    currency: 'GBP',
    callingCode: '500',
    flag: require('../assets/images/countries/gs.png'),
    name: {
      common: 'South Georgia',
      deu: 'Südgeorgien und die Südlichen Sandwichinseln',
      fra: 'Géorgie du Sud-et-les Îles Sandwich du Sud',
      hrv: 'Južna Georgija i otočje Južni Sandwich',
      ita: 'Georgia del Sud e Isole Sandwich Meridionali',
      jpn: 'サウスジョージア・サウスサンドウィッチ諸島',
      nld: 'Zuid-Georgia en Zuidelijke Sandwicheilanden',
      por: 'Ilhas Geórgia do Sul e Sandwich do Sul',
      rus: 'Южная Георгия и Южные Сандвичевы острова',
      spa: 'Islas Georgias del Sur y Sandwich del Sur',
      svk: 'Južná Georgia a Južné Sandwichove ostrovy',
      fin: 'Etelä-Georgia ja Eteläiset Sandwichsaaret',
      zho: '南乔治亚',
      isr: 'ג׳ורג׳יה הדרומית ואיי סנדוויץ׳ הדרומיים',
      ces: 'Jižní Georgie a Jižní Sandwichovy ostrovy',
      slk: 'Južná Georgia a Južné Sandwichove ostrovy',
      est: 'Lõuna-Georgia ja Lõuna-Sandwichi saared',
      pol: 'Georgia Południowa i Sandwich Południowy',
      urd: 'جنوبی جارجیا',
      kor: '조지아',
    },
  },
  SJ: {
    currency: 'NOK',
    callingCode: '4779',
    flag: require('../assets/images/countries/sj.png'),
    name: {
      common: 'Svalbard and Jan Mayen',
      deu: 'Spitzbergen',
      fra: 'Svalbard et Jan Mayen',
      hrv: 'Svalbard i Jan Mayen',
      ita: 'Svalbard e Jan Mayen',
      jpn: 'スヴァールバル諸島およびヤンマイエン島',
      nld: 'Svalbard en Jan Mayen',
      por: 'Ilhas Svalbard e Jan Mayen',
      rus: 'Шпицберген и Ян-Майен',
      spa: 'Islas Svalbard y Jan Mayen',
      svk: 'Svalbard a Jan Mayen',
      fin: 'Huippuvuoret',
      zho: '斯瓦尔巴特',
      isr: 'סוולבארד ויאן מאיין',
      ces: 'Špicberky a Jan Mayen',
      slk: 'Svalbard a Jan Mayen',
      est: 'Svalbard',
      pol: 'Svalbard i Jan Mayen',
      urd: 'سوالبارڈ اور جان میئن',
      kor: '스발바르 얀마옌 제도',
    },
  },
  SB: {
    currency: 'SBD',
    callingCode: '677',
    flag: require('../assets/images/countries/sb.png'),
    name: {
      common: 'Solomon Islands',
      deu: 'Salomonen',
      fra: 'Îles Salomon',
      hrv: 'Solomonski Otoci',
      ita: 'Isole Salomone',
      jpn: 'ソロモン諸島',
      nld: 'Salomonseilanden',
      por: 'Ilhas Salomão',
      rus: 'Соломоновы Острова',
      spa: 'Islas Salomón',
      svk: 'Salomonove ostrovy',
      fin: 'Salomonsaaret',
      zho: '所罗门群岛',
      isr: 'איי שלמה',
      ces: 'Šalamounovy ostrovy',
      slk: 'Salomonove ostrovy',
      est: 'Saalomoni Saared',
      pol: 'Wyspy Salomona',
      urd: 'جزائر سلیمان',
      kor: '솔로몬 제도',
    },
  },
  SL: {
    currency: 'SLL',
    callingCode: '232',
    flag: require('../assets/images/countries/sl.png'),
    name: {
      common: 'Sierra Leone',
      deu: 'Sierra Leone',
      fra: 'Sierra Leone',
      hrv: 'Sijera Leone',
      ita: 'Sierra Leone',
      jpn: 'シエラレオネ',
      nld: 'Sierra Leone',
      por: 'Serra Leoa',
      rus: 'Сьерра-Леоне',
      spa: 'Sierra Leone',
      svk: 'Sierra Leone',
      fin: 'Sierra Leone',
      zho: '塞拉利昂',
      isr: 'סיירה לאונה',
      ces: 'Sierra Leone',
      slk: 'Sierra Leone',
      est: 'Sierra Leone',
      pol: 'Sierra Leone',
      urd: 'سیرالیون',
      kor: '시에라리온',
    },
  },
  SV: {
    currency: 'SVC',
    callingCode: '503',
    flag: require('../assets/images/countries/sv.png'),
    name: {
      common: 'El Salvador',
      cym: 'El Salvador',
      deu: 'El Salvador',
      fra: 'Salvador',
      hrv: 'Salvador',
      ita: 'El Salvador',
      jpn: 'エルサルバドル',
      nld: 'El Salvador',
      por: 'El Salvador',
      rus: 'Сальвадор',
      spa: 'El Salvador',
      svk: 'Salvádor',
      fin: 'El Salvador',
      zho: '萨尔瓦多',
      isr: 'אל סלבדור',
      ces: 'Salvador',
      slk: 'Salvádor',
      est: 'El Salvador',
      pol: 'Salwador',
      urd: 'ایل سیلواڈور',
      kor: '엘살바도르',
    },
  },
  SM: {
    currency: 'EUR',
    callingCode: '378',
    flag: require('../assets/images/countries/sm.png'),
    name: {
      common: 'San Marino',
      deu: 'San Marino',
      fra: 'Saint-Marin',
      hrv: 'San Marino',
      ita: 'San Marino',
      jpn: 'サンマリノ',
      nld: 'San Marino',
      por: 'San Marino',
      rus: 'Сан-Марино',
      spa: 'San Marino',
      svk: 'San Maríno',
      fin: 'San Marino',
      zho: '圣马力诺',
      isr: 'סן מרינו',
      ces: 'San Marino',
      slk: 'San Maríno',
      est: 'San Marino',
      pol: 'San Marino',
      urd: 'سان مارینو',
      kor: '산마리노',
    },
  },
  SO: {
    currency: 'SOS',
    callingCode: '252',
    flag: require('../assets/images/countries/so.png'),
    name: {
      common: 'Somalia',
      deu: 'Somalia',
      fra: 'Somalie',
      hrv: 'Somalija',
      ita: 'Somalia',
      jpn: 'ソマリア',
      nld: 'Somalië',
      por: 'Somália',
      rus: 'Сомали',
      spa: 'Somalia',
      svk: 'Somálsko',
      fin: 'Somalia',
      zho: '索马里',
      isr: 'סומליה',
      ces: 'Somálsko',
      slk: 'Somálsko',
      est: 'Somaalia',
      pol: 'Somalia',
      urd: 'صومالیہ',
      kor: '소말리아',
    },
  },
  PM: {
    currency: 'EUR',
    callingCode: '508',
    flag: require('../assets/images/countries/pm.png'),
    name: {
      common: 'Saint Pierre and Miquelon',
      deu: 'Saint-Pierre und Miquelon',
      fra: 'Saint-Pierre-et-Miquelon',
      hrv: 'Sveti Petar i Mikelon',
      ita: 'Saint-Pierre e Miquelon',
      jpn: 'サンピエール島・ミクロン島',
      nld: 'Saint Pierre en Miquelon',
      por: 'Saint-Pierre e Miquelon',
      rus: 'Сен-Пьер и Микелон',
      spa: 'San Pedro y Miquelón',
      svk: 'Saint Pierre a Miquelon',
      fin: 'Saint-Pierre ja Miquelon',
      zho: '圣皮埃尔和密克隆',
      isr: 'סנט פייר ומיקלון',
      ces: 'Saint-Pierre a Miquelon',
      slk: 'Saint Pierre a Miquelon',
      est: 'Saint-Pierre ja Miquelon',
      pol: 'Saint-Pierre i Miquelon',
      urd: 'سینٹ پیئر و میکیلون',
      kor: '생피에르 미클롱',
    },
  },
  RS: {
    currency: 'RSD',
    callingCode: '381',
    flag: require('../assets/images/countries/rs.png'),
    name: {
      common: 'Serbia',
      deu: 'Serbien',
      fra: 'Serbie',
      hrv: 'Srbija',
      ita: 'Serbia',
      jpn: 'セルビア',
      nld: 'Servië',
      por: 'Sérvia',
      rus: 'Сербия',
      spa: 'Serbia',
      svk: 'Srbsko',
      fin: 'Serbia',
      zho: '塞尔维亚',
      isr: 'סרביה',
      ces: 'Srbsko',
      slk: 'Srbsko',
      est: 'Serbia',
      pol: 'Serbia',
      urd: 'سربیا',
      kor: '세르비아',
    },
  },
  SS: {
    currency: 'SSP',
    callingCode: '211',
    flag: require('../assets/images/countries/ss.png'),
    name: {
      common: 'South Sudan',
      deu: 'Südsudan',
      fra: 'Soudan du Sud',
      hrv: 'Južni Sudan',
      ita: 'Sudan del sud',
      jpn: '南スーダン',
      nld: 'Zuid-Soedan',
      por: 'Sudão do Sul',
      rus: 'Южный Судан',
      spa: 'Sudán del Sur',
      svk: 'Južný Sudán',
      fin: 'Etelä-Sudan',
      zho: '南苏丹',
      isr: 'דרום סודן',
      ces: 'Jižní Súdán',
      slk: 'Južný Sudán',
      est: 'Lõuna-Sudaan',
      pol: 'Sudan',
      urd: 'جنوبی سوڈان',
      kor: '남수단',
    },
  },
  ST: {
    currency: 'STD',
    callingCode: '239',
    flag: require('../assets/images/countries/st.png'),
    name: {
      common: 'São Tomé and Príncipe',
      deu: 'São Tomé und Príncipe',
      fra: 'São Tomé et Príncipe',
      hrv: 'Sveti Toma i Princip',
      ita: 'São Tomé e Príncipe',
      jpn: 'サントメ・プリンシペ',
      nld: 'Sao Tomé en Principe',
      por: 'São Tomé e Príncipe',
      rus: 'Сан-Томе и Принсипи',
      spa: 'Santo Tomé y Príncipe',
      svk: 'Svätý Tomáš a Princov ostrov',
      fin: 'São Téme ja Príncipe',
      zho: '圣多美和普林西比',
      isr: 'סאו טומה ופרינסיפה',
      ces: 'Svatý Tomáš a Princův ostrov',
      slk: 'Svätý Tomáš a Princov ostrov',
      est: 'São Tomé ja Príncipe',
      pol: 'Wyspy Świętego Tomasza i Książęca',
      urd: 'ساؤ ٹومے و پرنسپے',
      kor: '상투메 프린시페',
    },
  },
  SR: {
    currency: 'SRD',
    callingCode: '597',
    flag: require('../assets/images/countries/sr.png'),
    name: {
      common: 'Suriname',
      deu: 'Suriname',
      fra: 'Surinam',
      hrv: 'Surinam',
      ita: 'Suriname',
      jpn: 'スリナム',
      nld: 'Suriname',
      por: 'Suriname',
      rus: 'Суринам',
      spa: 'Surinam',
      svk: 'Surinam',
      fin: 'Suriname',
      zho: '苏里南',
      isr: 'סורינם',
      ces: 'Surinam',
      slk: 'Surinam',
      est: 'Suriname',
      pol: 'Surinam',
      urd: 'سرینام',
      kor: '수리남',
    },
  },
  SK: {
    currency: 'EUR',
    callingCode: '421',
    flag: require('../assets/images/countries/sk.png'),
    name: {
      common: 'Slovakia',
      deu: 'Slowakei',
      fra: 'Slovaquie',
      hrv: 'Slovačka',
      ita: 'Slovacchia',
      jpn: 'スロバキア',
      nld: 'Slowakije',
      por: 'Eslováquia',
      rus: 'Словакия',
      spa: 'República Eslovaca',
      svk: 'Slovensko',
      fin: 'Slovakia',
      zho: '斯洛伐克',
      isr: 'סלובקיה',
      ces: 'Slovensko',
      slk: 'Slovensko',
      est: 'Slovakkia',
      pol: 'Słowacja',
      urd: 'سلوواکیہ',
      kor: '슬로바키아',
    },
  },
  SI: {
    currency: 'EUR',
    callingCode: '386',
    flag: require('../assets/images/countries/si.png'),
    name: {
      common: 'Slovenia',
      deu: 'Slowenien',
      fra: 'Slovénie',
      hrv: 'Slovenija',
      ita: 'Slovenia',
      jpn: 'スロベニア',
      nld: 'Slovenië',
      por: 'Eslovénia',
      rus: 'Словения',
      spa: 'Eslovenia',
      svk: 'Slovinsko',
      fin: 'Slovenia',
      zho: '斯洛文尼亚',
      isr: 'סלובניה',
      ces: 'Slovinsko',
      slk: 'Slovinsko',
      est: 'Sloveenia',
      pol: 'Słowenia',
      urd: 'سلووینیا',
      kor: '슬로베니아',
    },
  },
  SE: {
    currency: 'SEK',
    callingCode: '46',
    flag: require('../assets/images/countries/se.png'),
    name: {
      common: 'Sweden',
      deu: 'Schweden',
      fra: 'Suède',
      hrv: 'Švedska',
      ita: 'Svezia',
      jpn: 'スウェーデン',
      nld: 'Zweden',
      por: 'Suécia',
      rus: 'Швеция',
      spa: 'Suecia',
      svk: 'šveédsko',
      fin: 'Ruotsi',
      zho: '瑞典',
      isr: 'שוודיה',
      ces: 'Švédsko',
      slk: 'Švédsko',
      est: 'Rootsi',
      pol: 'Szwecja',
      urd: 'سویڈن',
      kor: '스웨덴',
    },
  },
  SZ: {
    currency: 'SZL',
    callingCode: '268',
    flag: require('../assets/images/countries/sz.png'),
    name: {
      common: 'Eswatini',
      deu: 'Swasiland',
      fra: 'Swaziland',
      hrv: 'Svazi',
      ita: 'Swaziland',
      jpn: 'スワジランド',
      nld: 'Swaziland',
      por: 'Suazilândia',
      rus: 'Свазиленд',
      spa: 'Suazilandia',
      svk: 'Svazijsko',
      fin: 'Swazimaa',
      zho: '斯威士兰',
      isr: 'סווזילנד',
      ces: 'Svazijsko',
      slk: 'Svazijsko',
      est: 'Svaasimaa',
      pol: 'Suazi',
      urd: 'سوازی لینڈ',
      kor: '에스와티니',
    },
  },
  SX: {
    currency: 'ANG',
    callingCode: '1721',
    flag: require('../assets/images/countries/sx.png'),
    name: {
      common: 'Sint Maarten',
      deu: 'Sint Maarten',
      fra: 'Saint-Martin',
      ita: 'Sint Maarten',
      jpn: 'シント・マールテン',
      nld: 'Sint Maarten',
      por: 'São Martinho',
      rus: 'Синт-Мартен',
      spa: 'Sint Maarten',
      svk: 'Svätý Martin',
      fin: 'Sint Maarten',
      zho: '圣马丁岛',
      isr: 'סנט מארטן',
      ces: 'Svatý Martin (Nizozemsko)',
      hrv: 'Sveti Martin',
      slk: 'Sint Maarten',
      est: 'Sint Maarten',
      pol: 'Sint Maarten',
      urd: 'سنٹ مارٹن',
      kor: '신트마르턴',
    },
  },
  SC: {
    currency: 'SCR',
    callingCode: '248',
    flag: require('../assets/images/countries/sc.png'),
    name: {
      common: 'Seychelles',
      deu: 'Seychellen',
      fra: 'Seychelles',
      hrv: 'Sejšeli',
      ita: 'Seychelles',
      jpn: 'セーシェル',
      nld: 'Seychellen',
      por: 'Seicheles',
      rus: 'Сейшельские Острова',
      spa: 'Seychelles',
      svk: 'Seychely',
      fin: 'Seychellit',
      zho: '塞舌尔',
      isr: 'איי סיישל',
      ces: 'Seychely',
      slk: 'Seychely',
      est: 'Seišellid',
      pol: 'Seszele',
      urd: 'سیچیلیس',
      kor: '세이셸',
    },
  },
  SY: {
    currency: 'SYP',
    callingCode: '963',
    flag: require('../assets/images/countries/sy.png'),
    name: {
      common: 'Syria',
      deu: 'Syrien',
      fra: 'Syrie',
      hrv: 'Sirija',
      ita: 'Siria',
      jpn: 'シリア・アラブ共和国',
      nld: 'Syrië',
      por: 'Síria',
      rus: 'Сирия',
      spa: 'Siria',
      svk: 'Sýria',
      fin: 'Syyria',
      zho: '叙利亚',
      isr: 'סוריה',
      ces: 'Sýrie',
      slk: 'Sýria',
      est: 'Süüria',
      pol: 'Syria',
      urd: 'سوریہ',
      kor: '시리아',
    },
  },
  TC: {
    currency: 'USD',
    callingCode: '1649',
    flag: require('../assets/images/countries/tc.png'),
    name: {
      common: 'Turks and Caicos Islands',
      deu: 'Turks-und Caicosinseln',
      fra: 'Îles Turques-et-Caïques',
      hrv: 'Otoci Turks i Caicos',
      ita: 'Isole Turks e Caicos',
      jpn: 'タークス・カイコス諸島',
      nld: 'Turks-en Caicoseilanden',
      por: 'Ilhas Turks e Caicos',
      rus: 'Теркс и Кайкос',
      spa: 'Islas Turks y Caicos',
      svk: 'Turks a Caicos',
      fin: 'Turks-ja Caicossaaret',
      zho: '特克斯和凯科斯群岛',
      isr: 'איי טורקס וקאיקוס',
      ces: 'Turks a Caicos',
      slk: 'Turks a Caicos',
      est: 'Turks ja Caicos',
      pol: 'Turks i Caicos',
      urd: 'جزائر کیکس و ترکیہ',
      kor: '터크스 케이커스 제도',
    },
  },
  TD: {
    currency: 'XAF',
    callingCode: '235',
    flag: require('../assets/images/countries/td.png'),
    name: {
      common: 'Chad',
      cym: 'Tsiad',
      deu: 'Tschad',
      fra: 'Tchad',
      hrv: 'Čad',
      ita: 'Ciad',
      jpn: 'チャド',
      nld: 'Tsjaad',
      por: 'Chade',
      rus: 'Чад',
      spa: 'Chad',
      svk: 'Čad',
      fin: 'Tšad',
      zho: '乍得',
      isr: 'צ׳אד',
      ces: 'Čad',
      slk: 'Čad',
      est: 'Tšaad',
      pol: 'Czad',
      urd: 'چاڈ',
      kor: '차드',
    },
  },
  TG: {
    currency: 'XOF',
    callingCode: '228',
    flag: require('../assets/images/countries/tg.png'),
    name: {
      common: 'Togo',
      deu: 'Togo',
      fra: 'Togo',
      hrv: 'Togo',
      ita: 'Togo',
      jpn: 'トーゴ',
      nld: 'Togo',
      por: 'Togo',
      rus: 'Того',
      spa: 'Togo',
      svk: 'Togo',
      fin: 'Togo',
      zho: '多哥',
      isr: 'טוגו',
      ces: 'Togo',
      slk: 'Togo',
      est: 'Togo',
      pol: 'Togo',
      urd: 'ٹوگو',
      kor: '토고',
    },
  },
  TH: {
    currency: 'THB',
    callingCode: '66',
    flag: require('../assets/images/countries/th.png'),
    name: {
      common: 'Thailand',
      deu: 'Thailand',
      fra: 'Thaïlande',
      hrv: 'Tajland',
      ita: 'Tailandia',
      jpn: 'タイ',
      nld: 'Thailand',
      por: 'Tailândia',
      rus: 'Таиланд',
      spa: 'Tailandia',
      svk: 'Thajsko',
      fin: 'Thaimaa',
      zho: '泰国',
      isr: 'תאילנד',
      ces: 'Thajsko',
      slk: 'Thajsko',
      est: 'Tai',
      pol: 'Tajlandia',
      urd: 'تھائی لینڈ',
      kor: '태국',
    },
  },
  TJ: {
    currency: 'TJS',
    callingCode: '992',
    flag: require('../assets/images/countries/tj.png'),
    name: {
      common: 'Tajikistan',
      deu: 'Tadschikistan',
      fra: 'Tadjikistan',
      hrv: 'Tađikistan',
      ita: 'Tagikistan',
      jpn: 'タジキスタン',
      nld: 'Tadzjikistan',
      por: 'Tajiquistão',
      rus: 'Таджикистан',
      spa: 'Tayikistán',
      svk: 'Taǆikistan',
      fin: 'Tadžikistan',
      zho: '塔吉克斯坦',
      isr: 'טג׳יקיסטן',
      ces: 'Tádžikistán',
      slk: 'Taǆikistan',
      est: 'Tadžikistan',
      pol: 'Tadżykistan',
      urd: 'تاجکستان',
      kor: '타지키스탄',
    },
  },
  TK: {
    currency: 'NZD',
    callingCode: '690',
    flag: require('../assets/images/countries/tk.png'),
    name: {
      common: 'Tokelau',
      deu: 'Tokelau',
      fra: 'Tokelau',
      hrv: 'Tokelau',
      ita: 'Isole Tokelau',
      jpn: 'トケラウ',
      nld: 'Tokelau',
      por: 'Tokelau',
      rus: 'Токелау',
      spa: 'Islas Tokelau',
      svk: 'Tokelau',
      fin: 'Tokelau',
      zho: '托克劳',
      isr: 'טוקלאו',
      ces: 'Tokelau',
      slk: 'Tokelau',
      est: 'Tokelau',
      pol: 'Tokelau',
      urd: 'ٹوکیلاؤ',
      kor: '토켈라우',
    },
  },
  TM: {
    currency: 'TMT',
    callingCode: '993',
    flag: require('../assets/images/countries/tm.png'),
    name: {
      common: 'Turkmenistan',
      deu: 'Turkmenistan',
      fra: 'Turkménistan',
      hrv: 'Turkmenistan',
      ita: 'Turkmenistan',
      jpn: 'トルクメニスタン',
      nld: 'Turkmenistan',
      por: 'Turquemenistão',
      rus: 'Туркмения',
      spa: 'Turkmenistán',
      svk: 'Turkménsko',
      fin: 'Turkmenistan',
      zho: '土库曼斯坦',
      isr: 'טורקמניסטן',
      ces: 'Turkmenistán',
      slk: 'Turkménsko',
      est: 'Türkmenistan',
      pol: 'Turkmenistan',
      urd: 'ترکمانستان',
      kor: '투르크메니스탄',
    },
  },
  TL: {
    currency: 'USD',
    callingCode: '670',
    flag: require('../assets/images/countries/tl.png'),
    name: {
      common: 'Timor-Leste',
      deu: 'Timor-Leste',
      fra: 'Timor oriental',
      hrv: 'Istočni Timor',
      ita: 'Timor Est',
      jpn: '東ティモール',
      nld: 'Oost-Timor',
      por: 'Timor-Leste',
      rus: 'Восточный Тимор',
      spa: 'Timor Oriental',
      svk: 'Východný Timor',
      fin: 'Itä-Timor',
      zho: '东帝汶',
      isr: 'טימור לסטה',
      ces: 'Východní Timor',
      slk: 'Východný Timor',
      est: 'Ida-Timor',
      pol: 'Timor Wschodni',
      urd: 'مشرقی تیمور',
      kor: '동티모르',
    },
  },
  TO: {
    currency: 'TOP',
    callingCode: '676',
    flag: require('../assets/images/countries/to.png'),
    name: {
      common: 'Tonga',
      deu: 'Tonga',
      fra: 'Tonga',
      hrv: 'Tonga',
      ita: 'Tonga',
      jpn: 'トンガ',
      nld: 'Tonga',
      por: 'Tonga',
      rus: 'Тонга',
      spa: 'Tonga',
      svk: 'Tonga',
      fin: 'Tonga',
      zho: '汤加',
      isr: 'טונגה',
      ces: 'Tonga',
      slk: 'Tonga',
      est: 'Tonga',
      pol: 'Tonga',
      urd: 'ٹونگا',
      kor: '통가',
    },
  },
  TT: {
    currency: 'TTD',
    callingCode: '1868',
    flag: require('../assets/images/countries/tt.png'),
    name: {
      common: 'Trinidad and Tobago',
      deu: 'Trinidad und Tobago',
      fra: 'Trinité-et-Tobago',
      hrv: 'Trinidad i Tobago',
      ita: 'Trinidad e Tobago',
      jpn: 'トリニダード・トバゴ',
      nld: 'Trinidad en Tobago',
      por: 'Trinidade e Tobago',
      rus: 'Тринидад и Тобаго',
      spa: 'Trinidad y Tobago',
      svk: 'Trinidad a Tobago',
      fin: 'Trinidad ja Tobago',
      zho: '特立尼达和多巴哥',
      isr: 'טרינידד וטובגו',
      ces: 'Trinidad a Tobago',
      slk: 'Trinidad a Tobago',
      est: 'Trinidad ja Tobago',
      pol: 'Trynidad i Tobago',
      urd: 'ٹرینیڈاڈ و ٹوباگو',
      kor: '트리니다드 토바고',
    },
  },
  TN: {
    currency: 'TND',
    callingCode: '216',
    flag: require('../assets/images/countries/tn.png'),
    name: {
      common: 'Tunisia',
      deu: 'Tunesien',
      fra: 'Tunisie',
      hrv: 'Tunis',
      ita: 'Tunisia',
      jpn: 'チュニジア',
      nld: 'Tunesië',
      por: 'Tunísia',
      rus: 'Тунис',
      spa: 'Túnez',
      svk: 'Tunisko',
      fin: 'Tunisia',
      zho: '突尼斯',
      isr: 'טוניסיה',
      ces: 'Tunisko',
      slk: 'Tunisko',
      est: 'Tuneesia',
      pol: 'Tunezja',
      urd: 'تونس',
      kor: '튀니지',
    },
  },
  TR: {
    currency: 'TRY',
    callingCode: '90',
    flag: require('../assets/images/countries/tr.png'),
    name: {
      common: 'Turkey',
      deu: 'Türkei',
      fra: 'Turquie',
      hrv: 'Turska',
      ita: 'Turchia',
      jpn: 'トルコ',
      nld: 'Turkije',
      por: 'Turquia',
      rus: 'Турция',
      spa: 'Turquía',
      svk: 'Turecko',
      fin: 'Turkki',
      zho: '土耳其',
      isr: 'טורקיה',
      ces: 'Turecko',
      slk: 'Turecko',
      est: 'Türgi',
      pol: 'Turcja',
      urd: 'ترکی',
      kor: '터키',
    },
  },
  TV: {
    currency: 'AUD',
    callingCode: '688',
    flag: require('../assets/images/countries/tv.png'),
    name: {
      common: 'Tuvalu',
      deu: 'Tuvalu',
      fra: 'Tuvalu',
      hrv: 'Tuvalu',
      ita: 'Tuvalu',
      jpn: 'ツバル',
      nld: 'Tuvalu',
      por: 'Tuvalu',
      rus: 'Тувалу',
      spa: 'Tuvalu',
      svk: 'Tuvalu',
      fin: 'Tuvalu',
      zho: '图瓦卢',
      isr: 'טובאלו',
      ces: 'Tuvalu',
      slk: 'Tuvalu',
      est: 'Tuvalu',
      pol: 'Tuvalu',
      urd: 'تووالو',
      kor: '투발루',
    },
  },
  TW: {
    currency: 'TWD',
    callingCode: '886',
    flag: require('../assets/images/countries/tw.png'),
    name: {
      common: 'Taiwan',
      deu: 'Taiwan',
      fra: 'Taïwan',
      hrv: 'Tajvan',
      ita: 'Taiwan',
      jpn: '台湾（台湾省/中華民国）',
      nld: 'Taiwan',
      por: 'Ilha Formosa',
      rus: 'Тайвань',
      spa: 'Taiwán',
      svk: 'Taiwan',
      fin: 'Taiwan',
      isr: 'טייוואן',
      ces: 'Tchaj-wan',
      slk: 'Taiwan',
      est: 'Taiwan',
      pol: 'Tajwan',
      urd: 'تائیوان',
      kor: '대만',
    },
  },
  TZ: {
    currency: 'TZS',
    callingCode: '255',
    flag: require('../assets/images/countries/tz.png'),
    name: {
      common: 'Tanzania',
      deu: 'Tansania',
      fra: 'Tanzanie',
      hrv: 'Tanzanija',
      ita: 'Tanzania',
      jpn: 'タンザニア',
      nld: 'Tanzania',
      por: 'Tanzânia',
      rus: 'Танзания',
      spa: 'Tanzania',
      svk: 'Tanzánia',
      fin: 'Tansania',
      zho: '坦桑尼亚',
      isr: 'טנזניה',
      ces: 'Tanzanie',
      slk: 'Tanzánia',
      est: 'Tansaania',
      pol: 'Tanzania',
      urd: 'تنزانیہ',
      kor: '탄자니아',
    },
  },
  UG: {
    currency: 'UGX',
    callingCode: '256',
    flag: require('../assets/images/countries/ug.png'),
    name: {
      common: 'Uganda',
      deu: 'Uganda',
      fra: 'Ouganda',
      hrv: 'Uganda',
      ita: 'Uganda',
      jpn: 'ウガンダ',
      nld: 'Oeganda',
      por: 'Uganda',
      rus: 'Уганда',
      spa: 'Uganda',
      svk: 'Uganda',
      fin: 'Uganda',
      zho: '乌干达',
      isr: 'אוגנדה',
      ces: 'Uganda',
      slk: 'Uganda',
      est: 'Uganda',
      pol: 'Uganda',
      urd: 'یوگنڈا',
      kor: '우간다',
    },
  },
  UA: {
    currency: 'UAH',
    callingCode: '380',
    flag: require('../assets/images/countries/ua.png'),
    name: {
      common: 'Ukraine',
      deu: 'Ukraine',
      fra: 'Ukraine',
      hrv: 'Ukrajina',
      ita: 'Ucraina',
      jpn: 'ウクライナ',
      nld: 'Oekraïne',
      por: 'Ucrânia',
      rus: 'Украина',
      spa: 'Ucrania',
      svk: 'Ukrajina',
      fin: 'Ukraina',
      zho: '乌克兰',
      isr: 'אוקראינה',
      ces: 'Ukrajina',
      slk: 'Ukrajina',
      est: 'Ukraina',
      pol: 'Ukraina',
      urd: 'یوکرین',
      kor: '우크라이나',
    },
  },
  UM: {
    currency: 'USD',
    flag: require('../assets/images/countries/um.png'),
    name: {
      common: 'United States Minor Outlying Islands',
      deu: 'Kleinere Inselbesitzungen der Vereinigten Staaten',
      fra: 'Îles mineures éloignées des États-Unis',
      hrv: 'Mali udaljeni otoci SAD-a',
      ita: "Isole minori esterne degli Stati Uniti d'America",
      jpn: '合衆国領有小離島',
      nld: 'Kleine afgelegen eilanden van de Verenigde Staten',
      por: 'Ilhas Menores Distantes dos Estados Unidos',
      rus: 'Внешние малые острова США',
      spa: 'Islas Ultramarinas Menores de Estados Unidos',
      svk: 'Menšie odľahlé ostrovy USA',
      fin: 'Yhdysvaltain asumattomat saaret',
      zho: '美国本土外小岛屿',
      isr: 'האיים המרוחקים הקטנים של ארה״ב',
      ces: 'Menší odlehlé ostrovy USA',
      slk: 'Menšie odľahlé ostrovy USA',
      est: 'Ühendriikide hajasaared',
      pol: 'Dalekie Wyspy Mniejsze Stanów Zjednoczonych',
      urd: 'امریکی چھوٹے بیرونی جزائر',
      kor: '미국령 군소 제도',
    },
  },
  UY: {
    currency: 'UYI',
    callingCode: '598',
    flag: require('../assets/images/countries/uy.png'),
    name: {
      common: 'Uruguay',
      deu: 'Uruguay',
      fra: 'Uruguay',
      hrv: 'Urugvaj',
      ita: 'Uruguay',
      jpn: 'ウルグアイ',
      nld: 'Uruguay',
      por: 'Uruguai',
      rus: 'Уругвай',
      spa: 'Uruguay',
      svk: 'Uruguaj',
      fin: 'Uruguay',
      zho: '乌拉圭',
      isr: 'אורוגוואי',
      ces: 'Uruguay',
      slk: 'Uruguaj',
      est: 'Uruguay',
      pol: 'Urugwaj',
      urd: 'یوراگوئے',
      kor: '우루과이',
    },
  },
  US: {
    currency: 'USD',
    callingCode: '1',
    flag: require('../assets/images/countries/us.png'),
    name: {
      common: 'United States',
      deu: 'Vereinigte Staaten von Amerika',
      fra: 'États-Unis',
      hrv: 'Sjedinjene Američke Države',
      ita: "Stati Uniti d'America",
      jpn: 'アメリカ合衆国',
      nld: 'Verenigde Staten',
      por: 'Estados Unidos',
      rus: 'Соединённые Штаты Америки',
      spa: 'Estados Unidos',
      svk: 'Spojené štáty',
      fin: 'Yhdysvallat',
      zho: '美国',
      isr: 'ארצות הברית',
      ces: 'Spojené státy',
      slk: 'Spojené štáty americké',
      est: 'Ameerika Ühendriigid',
      pol: 'Stany Zjednoczone',
      urd: 'ریاستہائے متحدہ',
      kor: '미국',
    },
  },
  UZ: {
    currency: 'UZS',
    callingCode: '998',
    flag: require('../assets/images/countries/uz.png'),
    name: {
      common: 'Uzbekistan',
      deu: 'Usbekistan',
      fra: 'Ouzbékistan',
      hrv: 'Uzbekistan',
      ita: 'Uzbekistan',
      jpn: 'ウズベキスタン',
      nld: 'Oezbekistan',
      por: 'Uzbequistão',
      rus: 'Узбекистан',
      spa: 'Uzbekistán',
      svk: 'Uzbekistan',
      fin: 'Uzbekistan',
      zho: '乌兹别克斯坦',
      isr: 'אוזבקיסטן',
      ces: 'Uzbekistán',
      slk: 'Uzbekistan',
      est: 'Usbekistan',
      pol: 'Uzbekistan',
      urd: 'ازبکستان',
      kor: '우즈베키스탄',
    },
  },
  VA: {
    currency: 'EUR',
    callingCode: '3906698',
    flag: require('../assets/images/countries/va.png'),
    name: {
      common: 'Vatican City',
      deu: 'Vatikanstadt',
      fra: 'Cité du Vatican',
      hrv: 'Vatikan',
      ita: 'Città del Vaticano',
      jpn: 'バチカン市国',
      nld: 'Vaticaanstad',
      por: 'Cidade do Vaticano',
      rus: 'Ватикан',
      spa: 'Ciudad del Vaticano',
      svk: 'Vatikán',
      fin: 'Vatikaani',
      zho: '梵蒂冈',
      isr: 'הוותיקן',
      ces: 'Vatikán',
      slk: 'Vatikán',
      est: 'Vatikan',
      pol: 'Watykan',
      urd: 'ویٹیکن سٹی',
      kor: '바티칸',
    },
  },
  VC: {
    currency: 'XCD',
    callingCode: '1784',
    flag: require('../assets/images/countries/vc.png'),
    name: {
      common: 'Saint Vincent and the Grenadines',
      deu: 'Saint Vincent und die Grenadinen',
      fra: 'Saint-Vincent-et-les-Grenadines',
      hrv: 'Sveti Vincent i Grenadini',
      ita: 'Saint Vincent e Grenadine',
      jpn: 'セントビンセントおよびグレナディーン諸島',
      nld: 'Saint Vincent en de Grenadines',
      por: 'São Vincente e Granadinas',
      rus: 'Сент-Винсент и Гренадины',
      spa: 'San Vicente y Granadinas',
      svk: 'Svätý Vincent a Grenadíny',
      fin: 'Saint Vincent ja Grenadiinit',
      zho: '圣文森特和格林纳丁斯',
      isr: 'סנט וינסנט והגרנדינים',
      ces: 'Svatý Vincenc a Grenadiny',
      slk: 'Svätý Vincent a Grenadíny',
      est: 'Saint Vincent',
      pol: 'Saint Vincent i Grenadyny',
      urd: 'سینٹ وینسینٹ و گریناڈائنز',
      kor: '세인트빈센트 그레나딘',
    },
  },
  VE: {
    currency: 'VEF',
    callingCode: '58',
    flag: require('../assets/images/countries/ve.png'),
    name: {
      common: 'Venezuela',
      deu: 'Venezuela',
      fra: 'Venezuela',
      hrv: 'Venezuela',
      ita: 'Venezuela',
      jpn: 'ベネズエラ・ボリバル共和国',
      nld: 'Venezuela',
      por: 'Venezuela',
      rus: 'Венесуэла',
      spa: 'Venezuela',
      svk: 'Venezuela',
      fin: 'Venezuela',
      zho: '委内瑞拉',
      isr: 'ונצואלה',
      ces: 'Venezuela',
      slk: 'Venezuela',
      est: 'Venezuela',
      pol: 'Wenezuela',
      urd: 'وینیزویلا',
      kor: '베네수엘라',
    },
  },
  VG: {
    currency: 'USD',
    callingCode: '1284',
    flag: require('../assets/images/countries/vg.png'),
    name: {
      common: 'British Virgin Islands',
      deu: 'Britische Jungferninseln',
      fra: 'Îles Vierges britanniques',
      hrv: 'Britanski Djevičanski Otoci',
      ita: 'Isole Vergini Britanniche',
      jpn: 'イギリス領ヴァージン諸島',
      nld: 'Britse Maagdeneilanden',
      por: 'Ilhas Virgens',
      rus: 'Британские Виргинские острова',
      spa: 'Islas Vírgenes del Reino Unido',
      svk: 'Panenské ostrovy',
      fin: 'Neitsytsaaret',
      zho: '英属维尔京群岛',
      isr: 'איי הבתולה הבריטיים',
      ces: 'Britské Panenské ostrovy',
      slk: 'Panenské ostrovy',
      est: 'Briti Neitsisaared',
      pol: 'Brytyjskie Wyspy Dziewicze',
      urd: 'برطانوی جزائر ورجن',
      kor: '영국령 버진아일랜드',
    },
  },
  VI: {
    currency: 'USD',
    callingCode: '1340',
    flag: require('../assets/images/countries/vi.png'),
    name: {
      common: 'United States Virgin Islands',
      deu: 'Amerikanische Jungferninseln',
      fra: 'Îles Vierges des États-Unis',
      hrv: 'Američki Djevičanski Otoci',
      ita: 'Isole Vergini americane',
      jpn: 'アメリカ領ヴァージン諸島',
      nld: 'Amerikaanse Maagdeneilanden',
      por: 'Ilhas Virgens dos Estados Unidos',
      rus: 'Виргинские Острова',
      spa: 'Islas Vírgenes de los Estados Unidos',
      svk: 'Americké Panenské ostrovy',
      fin: 'Neitsytsaaret',
      zho: '美属维尔京群岛',
      isr: 'איי הבתולה של ארצות הברית',
      ces: 'Americké Panenské ostrovy',
      slk: 'Americké Panenské ostrovy',
      est: 'Neitsisaared, USA',
      pol: 'Wyspy Dziewicze Stanów Zjednoczonych',
      urd: 'امریکی جزائر ورجن',
      kor: '미국령 버진아일랜드',
    },
  },
  VN: {
    currency: 'VND',
    callingCode: '84',
    flag: require('../assets/images/countries/vn.png'),
    name: {
      common: 'Vietnam',
      deu: 'Vietnam',
      fra: 'Viêt Nam',
      hrv: 'Vijetnam',
      ita: 'Vietnam',
      jpn: 'ベトナム',
      nld: 'Vietnam',
      por: 'Vietname',
      rus: 'Вьетнам',
      spa: 'Vietnam',
      svk: 'Vietnam',
      fin: 'Vietnam',
      zho: '越南',
      isr: 'וייטנאם',
      ces: 'Vietnam',
      slk: 'Vietnam',
      est: 'Vietnam',
      pol: 'Wietnam',
      urd: 'ویتنام',
      kor: '베트남',
    },
  },
  VU: {
    currency: 'VUV',
    callingCode: '678',
    flag: require('../assets/images/countries/vu.png'),
    name: {
      common: 'Vanuatu',
      deu: 'Vanuatu',
      fra: 'Vanuatu',
      hrv: 'Vanuatu',
      ita: 'Vanuatu',
      jpn: 'バヌアツ',
      nld: 'Vanuatu',
      por: 'Vanuatu',
      rus: 'Вануату',
      spa: 'Vanuatu',
      svk: 'Vanuatu',
      fin: 'Vanuatu',
      zho: '瓦努阿图',
      isr: 'ונואטו',
      ces: 'Vanuatu',
      slk: 'Vanuatu',
      est: 'Vanuatu',
      pol: 'Vanuatu',
      urd: 'وانواتو',
      kor: '바누아투',
    },
  },
  WF: {
    currency: 'XPF',
    callingCode: '681',
    flag: require('../assets/images/countries/wf.png'),
    name: {
      common: 'Wallis and Futuna',
      deu: 'Wallis und Futuna',
      fra: 'Wallis-et-Futuna',
      hrv: 'Wallis i Fortuna',
      ita: 'Wallis e Futuna',
      jpn: 'ウォリス・フツナ',
      nld: 'Wallis en Futuna',
      por: 'Wallis e Futuna',
      rus: 'Уоллис и Футуна',
      spa: 'Wallis y Futuna',
      svk: 'Wallis a Futuna',
      fin: 'Wallis ja Futuna',
      zho: '瓦利斯和富图纳群岛',
      isr: 'איי ווליס ופוטונה',
      ces: 'Wallis a Futuna',
      slk: 'Wallis a Futuna',
      est: 'Wallis ja Futuna',
      pol: 'Wallis i Futuna',
      urd: 'والس و فتونہ',
      kor: '왈리스 퓌튀나',
    },
  },
  WS: {
    currency: 'WST',
    callingCode: '685',
    flag: require('../assets/images/countries/ws.png'),
    name: {
      common: 'Samoa',
      deu: 'Samoa',
      fra: 'Samoa',
      hrv: 'Samoa',
      ita: 'Samoa',
      jpn: 'サモア',
      nld: 'Samoa',
      por: 'Samoa',
      rus: 'Самоа',
      spa: 'Samoa',
      fin: 'Samoa',
      zho: '萨摩亚',
      isr: 'סמואה',
      ces: 'Samoa',
      slk: 'Samoa',
      est: 'Samoa',
      pol: 'Samoa',
      urd: 'سامووا',
      kor: '사모아',
    },
  },
  YE: {
    currency: 'YER',
    callingCode: '967',
    flag: require('../assets/images/countries/ye.png'),
    name: {
      common: 'Yemen',
      deu: 'Jemen',
      fra: 'Yémen',
      hrv: 'Jemen',
      ita: 'Yemen',
      jpn: 'イエメン',
      nld: 'Jemen',
      por: 'Iémen',
      rus: 'Йемен',
      spa: 'Yemen',
      svk: 'Jemen',
      fin: 'Jemen',
      zho: '也门',
      isr: 'תימן',
      ces: 'Jemen',
      slk: 'Jemen',
      est: 'Jeemen',
      pol: 'Jemen',
      urd: 'یمن',
      kor: '예멘',
    },
  },
  ZA: {
    currency: 'ZAR',
    callingCode: '27',
    flag: require('../assets/images/countries/za.png'),
    name: {
      common: 'South Africa',
      deu: 'Republik Südafrika',
      fra: 'Afrique du Sud',
      hrv: 'Južnoafrička Republika',
      ita: 'Sud Africa',
      jpn: '南アフリカ',
      nld: 'Zuid-Afrika',
      por: 'África do Sul',
      rus: 'Южно-Африканская Республика',
      spa: 'República de Sudáfrica',
      svk: 'Juhoafrická republika',
      fin: 'Etelä-Afrikka',
      zho: '南非',
      isr: 'דרום אפריקה',
      ces: 'Jihoafrická republika',
      slk: 'Juhoafrická republika',
      est: 'Lõuna-Aafrika Vabariik',
      pol: 'Południowa Afryka',
      urd: 'جنوبی افریقا',
      kor: '남아프리카',
    },
  },
  ZM: {
    currency: 'ZMW',
    callingCode: '260',
    flag: require('../assets/images/countries/zm.png'),
    name: {
      common: 'Zambia',
      deu: 'Sambia',
      fra: 'Zambie',
      hrv: 'Zambija',
      ita: 'Zambia',
      jpn: 'ザンビア',
      nld: 'Zambia',
      por: 'Zâmbia',
      rus: 'Замбия',
      spa: 'Zambia',
      svk: 'Zambia',
      fin: 'Sambia',
      zho: '赞比亚',
      isr: 'זמביה',
      ces: 'Zambie',
      slk: 'Zambia',
      est: 'Sambia',
      pol: 'Zambia',
      urd: 'زیمبیا',
      kor: '잠비아',
    },
  },
  ZW: {
    currency: 'ZWL',
    callingCode: '263',
    flag: require('../assets/images/countries/zw.png'),
    name: {
      common: 'Zimbabwe',
      deu: 'Simbabwe',
      fra: 'Zimbabwe',
      hrv: 'Zimbabve',
      ita: 'Zimbabwe',
      jpn: 'ジンバブエ',
      nld: 'Zimbabwe',
      por: 'Zimbabwe',
      rus: 'Зимбабве',
      spa: 'Zimbabue',
      svk: 'Zimbabwe',
      fin: 'Zimbabwe',
      zho: '津巴布韦',
      isr: 'זימבבואה',
      ces: 'Zimbabwe',
      slk: 'Zimbabwe',
      est: 'Zimbabwe',
      pol: 'Zimbabwe',
      urd: 'زمبابوے',
      kor: '짐바브웨',
    },
  },
};
